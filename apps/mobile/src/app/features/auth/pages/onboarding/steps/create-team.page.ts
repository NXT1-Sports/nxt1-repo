/**
 * @fileoverview Create Team Profile Step Page (Director/Coach)
...
@Component({
  selector: 'app-onboarding-create-team-step',
  standalone: true,
  imports: [
    IonContent,
    AuthShellComponent,
        ...
        <div authContent class="nxt1-onboarding-step">
          ...
          @if (contentReady()) {
            <nxt1-onboarding-create-team-step
              [teamData]="onboarding.createTeamProfileFormData()"
              [disabled]="onboarding.isLoading()"
              (teamChange)="onboarding.onCreateTeamProfileChange($event)"
            />
          }
        </div>
      </nxt1-auth-shell>
...
export class OnboardingCreateTeamStepPage {
