...
class GoogleAuthProviderHandler: NSObject {
    ...
    private func startSignInWithGoogleFlow(_ call: CAPPluginCall, isLink: Bool) {
        ...
        // Read GIDServerClientID from GoogleService-Info.plist so serverAuthCode is
        // scoped to the Web OAuth client — required for backend refresh_token exchange.
        // The build script (auto-switch-firebase-env.js) copies the correct plist
        // per environment (staging/production) automatically.
        var serverClientID = clientId
        if let plistPath = Bundle.main.path(forResource: "GoogleService-Info", ofType: "plist"),
           let plist = NSDictionary(contentsOfFile: plistPath),
           let webClientID = plist["GIDServerClientID"] as? String {
