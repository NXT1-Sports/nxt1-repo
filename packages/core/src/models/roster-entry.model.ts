...
// Map old TeamMember to RosterEntry
// ============================================

/**
 * Legacy TeamMember interface (from team-code.model.ts)
...
export interface LegacyTeamMember {
  ...
  role: string; // 'Administrative' | 'Athlete' | 'Coach' | 'Media'
  isVerify: boolean;
...
/**
 * Convert legacy TeamMember to RosterEntry
 */
export function teamMemberToRosterEntry(
  member: LegacyTeamMember,
  teamId: string,
  organizationId: string
): CreateRosterEntryInput {
  // Map old role strings to new RosterRole enum
  const roleMap: Record<string, RosterRole> = {
    Administrative: RosterRole.OWNER,
    Coach: RosterRole.HEAD_COACH,
    Athlete: RosterRole.ATHLETE,
    Media: RosterRole.MEDIA,
  };

  const role = roleMap[member.role] || RosterRole.ATHLETE;
  ...
  return {
    ...
    organizationId,
    role,
    status: member.isVerify ? RosterEntryStatus.ACTIVE : RosterEntryStatus.PENDING,
    positions: Array.isArray(member.position)
      ? member.position
      : member.position
        ? [member.position]
        : undefined,
...
/**
 * Convert RosterEntry to legacy TeamMember format
 * (for backward compatibility with existing frontend code)
 */
export function rosterEntryToTeamMember(entry: RosterEntry): LegacyTeamMember {
  const roleMap: Record<RosterRole, string> = {
    [RosterRole.OWNER]: 'Administrative',
    [RosterRole.HEAD_COACH]: 'Coach',
    [RosterRole.ASSISTANT_COACH]: 'Coach',
    [RosterRole.STAFF]: 'Coach',
    [RosterRole.ATHLETE]: 'Athlete',
    [RosterRole.STARTER]: 'Athlete',
    [RosterRole.BENCH]: 'Athlete',
    [RosterRole.JV]: 'Athlete',
    [RosterRole.MEDIA]: 'Media',
    [RosterRole.PARENT]: 'Athlete', // fallback
  };
  ...
  return {
    ...
    joinTime: entry.joinedAt,
    role: roleMap[entry.role] || 'Athlete',
    isVerify: entry.status === RosterEntryStatus.ACTIVE,
    position: entry.positions,
    classOf: entry.classOf,
    gpa: entry.gpa,
    profileLink: '', // can be constructed from userId if needed
    email: entry.email || '',
    phoneNumber: entry.phoneNumber || '',
    title: entry.role === RosterRole.HEAD_COACH ? 'Head Coach' : undefined,
  };
}
