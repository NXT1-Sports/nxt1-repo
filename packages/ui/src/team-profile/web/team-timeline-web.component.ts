...
@Component({
  ...
  template: `
    @if (effectiveFeed().length > 0) {
      ...
      <div class="madden-empty" data-testid="team-timeline-empty">
        ...
        @if (teamProfile.isTeamAdmin()) {
          <button
            ...
            data-testid="team-timeline-add-btn"
...
export class TeamTimelineWebComponent {
   ...
   * otherwise auto-converts TeamProfileService posts via mappers.
   */
  protected readonly effectiveFeed = computed<readonly FeedItem[]>(() => {
    ...
    return posts.map((p) => feedPostToFeedItem(teamPostToFeedPost(p, author)));
  });
