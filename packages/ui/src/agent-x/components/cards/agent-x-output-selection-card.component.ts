...
const OUTPUT_FORMAT_FAVICONS: Readonly<Partial<Record<AgentXOutputOption['formatTag'], string>>> = {
    ...
    >
      <div class="output-card__header">
        <div class="output-card__header-copy">
          @if (questionCount() > 1) {
            <span class="output-card__step-meta"
              >Question {{ currentQuestionNumber() }} of {{ questionCount() }}</span
            >
          }
                ...
                >
                  @if (shouldRenderOptionIcon(option)) {
                    @if (shouldShowFavicon(option)) {
                      ...
                      <nxt1-icon
                        [name]="resolvedIconName(option)"
                        [size]="optionIconSize(option)"
                      />
                    }
                  }
                </span>
...
export class AgentXOutputSelectionCardComponent {
  ...
  protected submit(): void {
    ...
    const firstCustomText = stepResponses.find((response) =>
  ...
  private buildStepResponse(
    ...
    const draft =
      ...
      ({
        ...
        customText: '',
        skipped: false,
      } satisfies OutputSelectionStepDraft);
    ...
    const optionTitleById = new Map(
      (step.options ?? []).map((option) => [option.id, option.title])
    );
