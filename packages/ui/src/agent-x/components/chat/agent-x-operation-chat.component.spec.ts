...
type DesktopQuickPromptVisibilityHelper = {
  ...
  inputValue: ReturnType<typeof signal<string>>;
  pendingSelectedAction: ReturnType<typeof signal<OperationQuickAction['selectedAction'] | null>>;
...
describe('AgentXOperationChatComponent composer reply routing', () => {
  ...
  it('does not treat expired active approval yields as composer-reply targets', () => {
    ...
    component.messages = () => [];
    component.activeYieldState = signal({
      reason: 'needs_approval',
      promptToUser: 'Review and approve this email.',
      pendingToolCall: {
        toolName: 'send_email',
        toolInput: {
          operationId: 'op-expired-approval',
          toEmail: 'john@nxt1sports.com',
          subject: 'Agent X update',
        },
      },
    ...
    expect(component.pendingComposerReplyTarget()).toBeNull();
    expect(component.isAwaitingComposerReply()).toBe(false);
  });
});
