...
export class AgentXOperationChatMessageFacade {
  ...
  private hasGeneratedMediaReloadSignal(message: OperationMessage | undefined): boolean {
    ...
    const mediaSignalPattern =
      /https?:\/\/[^\s)\]"'<>]+(?:\.(?:m3u8|mov|mp4|m4v|webm|png|jpe?g|gif|webp|avif|pdf|xlsx|pptx)|(?:firebasestorage|storage)\.googleapis\.com|(?:cloudflarestream|videodelivery)\.net|\/media-proxy\/)/i;
    ...
    this.messages.update((messages) => {
      ...
      const yieldMessage: OperationMessage = separatesTypingPayload
        ? {
            ...
            ...(syntheticAskUserCard
              ? { parts: [{ type: 'card', card: syntheticAskUserCard }] }
              : {}),
          }
        : {
            id: messageId,
