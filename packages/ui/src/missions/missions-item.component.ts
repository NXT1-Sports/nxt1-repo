/**
 * @fileoverview Missions Item Component - Single Task Row
 * @module @nxt1/ui/missions
 * @version 1.0.0
 *
 * Interactive task/mission item with checkbox, points, and status indicators.
...
@Component({
  selector: 'nxt1-missions-item',
  standalone: true,
  imports: [CommonModule, IonIcon, IonRippleEffect, IonSpinner],
  template: `
    <div
      class="mission-item"
      [class.mission-item--completed]="isCompleted()"
      [class.mission-item--locked]="isLocked()"
      [class.mission-item--in-progress]="isInProgress()"
      [class.mission-item--featured]="mission().featured"
      [class.mission-item--critical]="mission().priority === 'critical'"
      [class.mission-item--high]="mission().priority === 'high'"
      role="listitem"
    ...
    >
      ...
      <button
        type="button"
        class="mission-item__checkbox"
        [disabled]="isLocked() || isCompleting()"
        (click)="handleCheckboxClick($event)"
        [attr.aria-label]="isCompleted() ? 'Completed' : 'Mark as complete'"
      >
        @if (isCompleting()) {
          <ion-spinner name="crescent" class="mission-item__spinner"></ion-spinner>
        } @else if (isCompleted()) {
          <ion-icon name="checkmark-circle" class="mission-item__check-icon"></ion-icon>
        } @else if (isLocked()) {
          <ion-icon name="lock-closed-outline" class="mission-item__lock-icon"></ion-icon>
        } @else {
          <div class="mission-item__checkbox-empty">
            @if (hasProgress()) {
              <svg class="mission-item__mini-progress" viewBox="0 0 24 24">
                <circle
                  cx="12"
                  cy="12"
                  r="10"
      ...
      <!-- Content -->
      <div class="mission-item__content" (click)="handleContentClick()">
        <!-- Title Row -->
        <div class="mission-item__title-row">
          <div class="mission-item__icon-wrapper" [style.--icon-color]="priorityColor()">
            <ion-icon [name]="mission().icon"></ion-icon>
          </div>
          <h4 class="mission-item__title" [class.mission-item__title--completed]="isCompleted()">
            {{ mission().title }}
          </h4>
        </div>
        ...
        <!-- Description -->
        @if (showDescription()) {
          <p class="mission-item__description">{{ mission().description }}</p>
        }
        ...
        <div class="mission-item__meta">
          ...
          @if (mission().socialProof && !isCompleted()) {
            <span class="mission-item__social-proof">
          ...
          @if (hasProgress() && !isCompleted()) {
            <span class="mission-item__progress-text"> {{ mission().progress }}% done </span>
          }

          <!-- Expiration -->
          @if (isExpiringSoon()) {
            <span class="mission-item__expiring">
      ...
      <!-- Trailing Section -->
      <div class="mission-item__trailing">
        <!-- Points Badge -->
        <div class="mission-item__points" [class.mission-item__points--earned]="isCompleted()">
          @if (!isCompleted()) {
            <ion-icon name="star" class="mission-item__points-icon"></ion-icon>
          }
        ...
        @if (mission().quickAction && !isCompleted() && !isLocked()) {
          <button
            type="button"
            class="mission-item__quick-action"
      ...
      @if (mission().featured && !isCompleted()) {
        <div class="mission-item__featured-badge">
  ...
  styles: [
    `
      /* ============================================
       MISSION ITEM - Gamified Task Row
      ...
      .mission-item__checkbox:not(:disabled):hover {
        transform: scale(1.1);
      }

      .mission-item__checkbox:not(:disabled):active {
        transform: scale(0.95);
      }

      .mission-item__checkbox:disabled {
        cursor: not-allowed;
      }

      .mission-item__checkbox-empty {
        width: 24px;
        height: 24px;
        border-radius: 50%;
      ...
      .mission-item__checkbox:hover .mission-item__checkbox-empty {
      ...
      .mission-item__lock-icon {
        font-size: 20px;
        color: var(--nxt1-color-text-tertiary, rgba(255, 255, 255, 0.5));
      }

      .mission-item__spinner {
        width: 24px;
        height: 24px;
        --color: var(--nxt1-color-primary, #ccff00);
      }

      .mission-item__mini-progress {
        position: absolute;
        inset: -2px;
        width: calc(100% + 4px);
        height: calc(100% + 4px);
      ...
      .mission-item__icon-wrapper ion-icon {
        font-size: 14px;
      ...
      .mission-item__time ion-icon,
      .mission-item__social-proof ion-icon {
        font-size: 12px;
      }
      ...
      .mission-item__expiring ion-icon {
        font-size: 12px;
      }
      ...
      .mission-item__points-icon {
        font-size: 12px;
      }

      .mission-item__quick-action {
        width: 32px;
        height: 32px;
        display: flex;
        align-items: center;
      ...
      .mission-item__quick-action:hover {
        background: var(--nxt1-color-surface-300, rgba(255, 255, 255, 0.08));
      }

      .mission-item__quick-action ion-icon {
        font-size: 16px;
        color: var(--nxt1-color-text-secondary, rgba(255, 255, 255, 0.7));
      }
      ...
      .mission-item__featured-badge ion-icon {
        font-size: 10px;
      }
    `,
  ],
...
export class MissionsItemComponent {
  // ============================================
  // INPUTS
  // ============================================
