...
@Component({
  ...
  template: `
    ...
    >
      ...
      @if (!report() && (intel.isLoading() || isResyncBlocked())) {
        ...
        <div class="intel-stage" [attr.data-testid]="testIds.REPORT_CONTAINER">
          @if (canGenerate()) {
            <div class="resync-bar">
              <button
                type="button"
                class="resync-bar__btn"
                [disabled]="isResyncBlocked()"
                (click)="onResync()"
              >
                <nxt1-icon name="refresh-outline" [size]="14" />
                {{ intel.isBackgroundJobRunning() ? 'Resyncing...' : resyncLabel() }}
              </button>
            </div>
          }
          <!-- ── Active Section Card ── -->
...
export class TeamIntelComponent {
  ...
  protected readonly isResyncBlocked = computed(
    () => this.intel.isGenerating() || this.intel.isPendingGeneration()
  );
  protected readonly resyncLabel = computed(() => {
    const activeSection = this.activeSection();

    if (activeSection === 'agent_overview') {
      return 'Resync Intel';
    }

    const sectionTitle = this.activeSectionData()?.title;
    return sectionTitle ? `Resync ${sectionTitle}` : 'Resync Intel';
  });
