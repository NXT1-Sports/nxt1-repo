...
import { IonTabBar, IonTabButton, IonFooter } from '@ionic/angular/standalone';
                ...
                @if (isAgentXTab(tab)) {
                  ...
                  >
                    <nxt1-avatar
                      ...
                      [defaultIcon]="profileAvatarIsTeam() ? 'shield' : ''"
                      [customSize]="32"
                      [showSkeleton]="false"
...
// Sticky CTA footer for routed Ionic pages.
// Must be a direct sibling of <ion-content> — never nested inside it.
// ============================================
...
@Component({
  ...
  styles: [
    `
      ...
      .action-footer__inner--1 .action-footer__btn {
        flex: 1 1 100%;
      }
      .action-footer__btn {
        flex: 1;
      ...
      .action-footer__btn:active {
        transform: scale(0.97);
      }
      .action-footer__btn--secondary {
...
export class NxtActionFooterComponent {
  readonly buttons = input<ActionFooterButton[]>([]);
  protected readonly innerClass = computed(
    () => `action-footer__inner action-footer__inner--${this.buttons().length}`
  );
}
