/**
 * @fileoverview Shared Stats Dashboard Component
 * @module @nxt1/ui/components/stats-dashboard
 * @version 1.0.0
 *
 * Pure presentational component — renders game log tables, career/season
 * stats, and top-stats comparison bars.
 *
 * Zero service dependencies. The parent shell maps its domain data
 * (ProfileSeasonGameLog[], AthleticStatsCategory[]) and passes it via inputs.
 *
...
interface StatsComparisonItem {
  ...
  /** Whether the average value comes from real backend data vs. synthetic estimate */
  readonly hasRealAverage: boolean;
}

@Component({
  selector: 'nxt1-stats-dashboard',
  standalone: true,
  ...
  template: `
    <section class="madden-tab-section stats-board" aria-labelledby="stats-heading">
      ...
      @if (gameLogs().length === 0 && athleticStats().length === 0) {
        <div class="madden-empty">
          ...
          <p>{{ emptyMessage() }}</p>
          @if (showAddButton()) {
            <button type="button" class="madden-cta-btn" (click)="addStats.emit()">
              Add Stats
            </button>
          }
        </div>
        ...
        @if (activeGameLog(); as gl) {
          ...
          <!-- ═══ Game Log Table (single-season only, when games exist) ═══ -->
          @if (!isCareerMode() && gl.games.length > 0) {
...
export class StatsDashboardComponent {
  ...
  /** Game log data (seasons with per-game entries and totals). */
  readonly gameLogs = input<readonly ProfileSeasonGameLog[]>([]);

  /** Athletic stats categories for the Top Stats comparison bars. */
  readonly athleticStats = input<readonly AthleticStatsCategory[]>([]);

  /** Display name in the comparison legend (e.g., athlete first name or team name). */
  readonly entityName = input('Athlete');

  /** Whether the dashboard is in "own profile" context (affects empty state CTA). */
  readonly showAddButton = input(false);

  /** Empty-state message. */
  readonly emptyMessage = input('No stats have been recorded yet.');

  /**
   * Active side tab from parent — used for season/team-type filtering.
   * Prefixed values: "school-season-2025-2026", "club-season-2024-2025", "career-school", etc.
   */
  readonly activeSideTab = input('');

  // ── Outputs ──

  /** Emitted when "Add Stats" button is clicked. */
  readonly addStats = output<void>();
  ...
  // ── Career mode detection (from activeSideTab) ──

  protected readonly isCareerMode = computed(() => {
    const tab = this.activeSideTab();
    return tab.startsWith('career-') || tab === '' || tab === 'career';
  });

  private readonly activeTeamType = computed<'school' | 'club'>(() => {
    const tab = this.activeSideTab();
    if (tab.includes('club')) return 'club';
    return 'school';
  });
  ...
  // ── Filtered game logs based on activeSideTab ──

  private readonly filteredGameLogs = computed(() => {
    const teamType = this.activeTeamType();
    return this.gameLogs().filter((l) => (l.teamType ?? 'school') === teamType);
  });
  ...
  protected readonly gameLogCategoriesForSeason = computed<readonly string[]>(() => {
    ...
    const activeSeason = this.activeSeasonLabel();
    if (!activeSeason) {
      // No specific season — show all unique categories
      const seen = new Set<string>();
      const cats: string[] = [];
      for (const log of logs) {
        if (!seen.has(log.category)) {
  ...
  protected readonly activeGameLog = computed<ProfileSeasonGameLog | null>(() => {
    ...
    const activeSeason = this.activeSeasonLabel();
    if (activeSeason) {
      const match = logs.find((l) => l.season === activeSeason && l.category === activeCategory);
      return match ?? null;
    }

    // No specific season — return first matching category
    return logs.find((l) => l.category === activeCategory) ?? null;
  });

  protected readonly careerSeasonLogs = computed<readonly ProfileSeasonGameLog[]>(() => {
    if (!this.isCareerMode()) return [];
    const logs = this.filteredGameLogs();
    const categories = this.gameLogCategoriesForSeason();
    const catIdx = this._activeGameLogCategoryIdx();
    const activeCategory = categories[catIdx] ?? categories[0];
  ...
  private readonly activeStatCategory = computed(() => {
    const cats = this.athleticStats();
    return cats[0] ?? null;
  });

  /** Whether any comparison item has a real (non-synthetic) national average */
  protected readonly hasAnyRealAverage = computed(() =>
    this.statsComparisonItems().some((item) => item.hasRealAverage)
  );
  ...
  protected readonly statsComparisonItems = computed<readonly StatsComparisonItem[]>(() => {
    ...
    return comparisonSource.map((stat) => {
      const playerNumeric = Math.max(0, this.parseNumericStatValue(stat.value));
      const resolved = this.resolveComparisonAverage(stat);

      return {
        label: stat.label,
        playerDisplay: `${stat.value}${stat.unit ?? ''}`,
        averageDisplay: resolved.hasReal ? this.formatComparisonAverage(stat, resolved.value) : '',
        playerPercent: this.toBarPercent(playerNumeric, maxValue),
        averagePercent: resolved.hasReal ? this.toBarPercent(resolved.value, maxValue) : 0,
        hasRealAverage: resolved.hasReal,
      };
    });
  });
  ...
  private buildCareerGameLog(
    ...
    for (const log of catLogs) {
      const yearMatch = log.season.match(/\d{4}/);
      const shortYear = yearMatch ? yearMatch[0].slice(2) : '';
      for (const game of log.games) {
        allGames.push({ ...game, date: shortYear ? `${game.date}/${shortYear}` : game.date });
      }
    }
    ...
    // Format all large numeric totals with commas for readability
    for (const key of Object.keys(careerTotalStats)) {
      const val = careerTotalStats[key];
      if (typeof val === 'number' && val >= 1000) {
        careerTotalStats[key] = this.formatNumberWithCommas(val);
      }
    }
    ...
    const careerRecord =
      totalTies > 0 ? `${totalWins}-${totalLosses}-${totalTies}` : `${totalWins}-${totalLosses}`;
  ...
  /**
   * Resolves comparison average for a stat.
   * Returns the real backend average if available, otherwise indicates no data.
   * Never fabricates synthetic averages — only uses real data from the backend.
   */
  private resolveComparisonAverage(stat: AthleticStat): {
    readonly value: number;
    readonly hasReal: boolean;
  } {
    const explicitAverage =
      stat.nationalAverage ?? stat.nationalAvg ?? stat.average ?? stat.avg ?? stat.benchmark;

    const parsedExplicit = this.parseNumericStatValue(explicitAverage);
    if (parsedExplicit > 0) {
      return { value: parsedExplicit, hasReal: true };
    }

    // No real average data available — don't fabricate synthetic values
    return { value: 0, hasReal: false };
  }
