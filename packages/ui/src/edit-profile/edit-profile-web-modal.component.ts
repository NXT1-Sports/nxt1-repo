/**
 * @fileoverview Edit Profile Web Modal Wrapper
 * @module @nxt1/ui/edit-profile
 * @version 1.0.0
 *
 * Thin wrapper around EditProfileShellComponent for use inside
 * NxtOverlayService on desktop web.
 *
 * Why a wrapper?
 * - NxtOverlayService auto-subscribes to `close` output to dismiss.
 * - EditProfileShellComponent emits `close` (cancel) and `save` (success)
...
@Component({
  selector: 'nxt1-edit-profile-web-modal',
  standalone: true,
  imports: [EditProfileShellComponent, NxtModalHeaderComponent],
  changeDetection: ChangeDetectionStrategy.OnPush,
  template: `
    <div class="nxt1-edit-web-modal">
      ...
      >
        <button
          ...
          [class.nxt1-web-modal-save-btn--active]="profileService.hasUnsavedChanges()"
        ...
        >
          @if (profileService.isSaving()) {
            ...
            <span>{{ profileService.hasUnsavedChanges() ? 'Save' : 'Done' }}</span>
          }
        </button>
      </nxt1-modal-header>
...
export class EditProfileWebModalComponent {
  readonly userId = input<string>();
  readonly sportIndex = input<number>();
  readonly connectProviderCallback = input<((provider: InboxEmailProvider) => void) | undefined>();
  readonly searchTeams = input<SearchTeamsFn | undefined>();
  ...
  private readonly shellRef = viewChild(EditProfileShellComponent);

  private readonly modal = inject(NxtModalService);
  private readonly logger = inject(NxtLoggingService).child('EditProfileWebModal');

  /** Exposed to template for save-button state bindings. */
  protected readonly profileService = inject(EditProfileService);
