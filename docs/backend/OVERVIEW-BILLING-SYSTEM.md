...
- `APPLE_BUNDLE_ID` — used by `iap.routes.ts`; set to `com.nxt1sports.nxt1`
- `APPLE_APP_ID` — used by `iap.routes.ts`; set to `6446410344`
- `APPLE_SHARED_SECRET` — fallback secret; set
- `HELICONE_API_KEY` — used by `modules/billing/helicone.service.ts` and
  `modules/agent/llm/openrouter.service.ts`; required when Helicone-backed cost
  attribution or proxy routing is enabled
- `STRIPE_SECRET_KEY` — used by `stripe.service.ts`; production key is set
- `STRIPE_TEST_SECRET_KEY` — used by `stripe.service.ts`; staging key is set |
