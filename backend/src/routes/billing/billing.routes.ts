  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const costCents = isDynamic
        ...
        : getUnitCost(feature) * Number(quantity);
 ...
 * Get list of billable features observed in usage history.
 */
router.get('/usage/features', async (_req: Request, res: Response) => {
  try {
    const features = await UsageEventModel.distinct('feature');

    res.json({
      success: true,
      features: features.filter((feature): feature is string => typeof feature === 'string').sort(),
    });
  } catch (error) {
    logger.error('[GET /usage/features] Failed to get dynamic feature list', { error });
    res.status(500).json({
      success: false,
      error: 'Failed to get usage features',
    });
  }
});
...
router.get('/budget', appGuard, async (req: Request, res: Response) => {
  try {
    ...
    const hasOrganizationBilling = ctx.organizationId
      ? await hasConfiguredOrganizationBilling(db, ctx.organizationId)
      : false;
 ...
 * Get current pricing config (default multiplier + coordinator/feature overrides).
 ...
 * `featureOverrides` remains the stored field name, but coordinator IDs
 * like `brand_coordinator` should be the default override keys for Agent X.
 *
...
router.post('/cron/expire-stale-holds', cronGuard, async (req: Request, res: Response) => {
  try {
    const db = req.firebase?.db;
    if (!db) {
    ...
    logger.info('[POST /cron/expire-stale-holds] Completed', { expiredCount });
