...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (billingMode === 'organization' && orgTarget.type !== 'organization') {
        return res.status(400).json({
          error: 'Organization billing is not available for this account',
        });
      }
      ...
      const hasOrganizationBilling = freshCtx.organizationId
        ? await hasConfiguredOrganizationBilling(db, freshCtx.organizationId)
        : false;
