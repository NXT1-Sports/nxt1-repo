 ...
 * @module @nxt1/backend/routes/agent-x/operations-log.helpers
 *
...
/**
 * Returns `true` only for time-based recurring operations.
 *
 * We intentionally do not classify all non-user origins as scheduled. Some
 * operations are backend-triggered (`database_event`, `webhook`, `agent_chain`)
 * but are still one-off tasks rather than recurring scheduled runs.
 */
export function isScheduledOrigin(origin: AgentJobOrigin): boolean {
