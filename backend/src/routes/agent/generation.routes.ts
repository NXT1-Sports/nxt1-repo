    ...
    try {
      ...
      if (playbookRef) {
        ...
        if (!requestedPlaybook.exists) {
          res.status(404).json({ success: false, error: 'Requested playbook not found' });
          return;
        }
      } else {
        ...
        playbookRef = latestPlaybook.docs[0].ref;
      }
      ...
      if (status === 'complete' || status === 'snoozed') {
        const checkDoc = await playbookRef.get();
        if (checkDoc.exists) {
          const allItems = (checkDoc.data()?.['items'] ?? []) as ShellWeeklyPlaybookItem[];
          ...
          try {
            const goalHistoryRef = db.collection('Users').doc(user.uid).collection('goal_history');
            ...
            logger.warn('Failed to sync goal history after item completion', {
              userId: user.uid,
              playbookId: playbookRef.id,
