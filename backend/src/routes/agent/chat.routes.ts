...
function buildSelectedContextResumeGuard(
  ...
  const filmReviewId =
    typeof originalContext['filmReviewId'] === 'string'
  ...
  const selectedFilmTitle =
    typeof originalContext['selectedFilmTitle'] === 'string'
      ? originalContext['selectedFilmTitle'].trim()
      : '';
  const intentMentionsFilm =
    /\bfull\s+game|film\s+review|selected\s+(?:film|plays|clips)|\bwk\s*\d+\b/i.test(intent ?? '');
...
function normalizeOutputSelectionOptionRecord(option: unknown): {
  ...
  return {
    ...
    isCustomInput: record['isCustomInput'] === true || formatTag === 'CUSTOM',
    ...(formatTag ? { formatTag } : {}),
  };
}
...
function normalizeOutputSelectionSteps(
  ...
  const options = (Array.isArray(toolInput['options']) ? toolInput['options'] : [])
    .map((option) => normalizeOutputSelectionOptionRecord(option))
    .filter(
      (option): option is NonNullable<ReturnType<typeof normalizeOutputSelectionOptionRecord>> =>
        option !== null && !option.isCustomInput
    )
    .map((option) => ({
      id: option.id,
  ...
  return [
    {
      id: 'step_1',
      prompt:
        typeof toolInput['prompt'] === 'string' ? toolInput['prompt'].trim() : 'Choose an option',
...
function normalizeOutputSelectionChoice(
  ...
  if (rawStepResponses.length > 0) {
    ...
    for (const rawResponse of rawStepResponses) {
      ...
      const customText =
        typeof rawResponse.customText === 'string' ? rawResponse.customText.trim() : '';
      const skipped = rawResponse.skipped === true;

      if (customText.length > 2000) {
        return {
          status: 400,
          error: `customText must be 2000 characters or less for step ${stepId}`,
        };
      }
...
function formatSelectedOutputOptions(options: readonly NormalizedOutputSelectionOption[]): string {
  return options
    .map((option) => `${option.title}${option.formatTag ? ` (${option.formatTag})` : ''}`)
    .join(', ');
}

function buildSelectedOutputLaneInstruction(choice: NormalizedOutputSelectionChoice): string {
...
function buildOutputIntentFromChoice(
  choice: NormalizedOutputSelectionChoice
): AgentOutputIntent | null {
  const lanes = new Set<AgentOutputDeliveryLane>();

  for (const option of choice.selectedOptions) {
    const searchable = `${option.id} ${option.title}`.toLowerCase();
    switch (option.formatTag) {
      case 'PDF':
      ...
      case 'CSV':
        ...
        break;
      default:
        if (/\bchat[_\s-]?summary\b|\bchat only\b|\bconversation\b/.test(searchable)) {
          lanes.add('chat_only');
        }
    }
  }
  ...
  return {
    ...
    selectedOptionTitles: choice.selectedOptionTitles,
  };
}

function buildOutputIntentFromAskUserReply(userResponse: string): AgentOutputIntent | null {
  const normalized = userResponse.toLowerCase().replace(/[_-]+/g, ' ');
  const lanes = new Set<AgentOutputDeliveryLane>();
  ...
  if (/\bchat[_\s-]?summary\b|\bchat only\b/.test(normalized)) lanes.add('chat_only');

  if (lanes.size === 0) return null;
  return { lanes: Array.from(lanes), source: 'ask_user_reply' };
}

function buildAskUserDeliveryResumeGuard(userResponse: string): ResumeMessageShape | null {
  const normalized = userResponse.toLowerCase().replace(/[_-]+/g, ' ');
  ...
  if (/\bprintable\s+pdf\b|\bprint ready\s+pdf\b|\bprint\s+pdf\b/.test(normalized)) {
    return {
      role: 'system',
      content: [
        'OUTPUT SELECTION GUARD: The user just answered an ask_user delivery/output-format step. This selection overrides any earlier best-fit/default/deck guidance.',
        'Selected output(s): Printable PDF (PDF).',
        buildSelectedOutputLaneInstruction({
          selectedOptionIds: ['printable-pdf'],
          selectedOptions: [
            {
              id: 'printable-pdf',
              title: 'Printable PDF',
...
router.post('/threads/:threadId/actions', appGuard, async (req: Request, res: Response) => {
  try {
    ...
    if (body.actionType === 'output_selection_choice') {
      ...
      const outputIntent = buildOutputIntentFromChoice(normalizedChoice);
      ...
      await jobRepository.withDb(db).markCompleted(resolvedOperationId, {
        ...
        data: {
          ...
          ...(normalizedChoice.stepResponses
            ? {
                stepResponses: normalizedChoice.stepResponses,
                yieldResolvedText: userSelectionMessage,
              }
            : {}),
        },
      });
    ...
    if (body.actionType === 'ask_user_reply') {
      ...
      const selectedContextGuard = buildSelectedContextResumeGuard(originalContext, jobDoc.intent);
      const askUserDeliveryGuard = buildAskUserDeliveryResumeGuard(trimmedComposedUserResponse);
      const outputIntent = buildOutputIntentFromAskUserReply(trimmedComposedUserResponse);
      ...
      const resumedMessages = resumeFromPausedState
        ...
        : [
            ...
            ...(selectedContextGuard ? [selectedContextGuard] : []),
            ...(askUserDeliveryGuard ? [askUserDeliveryGuard] : []),
          ];
