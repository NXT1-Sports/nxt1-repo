...
function getSubject(
  ...
  return sequenceStep === 'follow_up'
    ? `Following Up On The Invite For ${label}`
    : `An Invite For ${label}`;
}
...
export function buildB2BPartnerBrandAwarenessEmail(
  ...
  const campaignKey = sequenceStep === 'follow_up' ? FOLLOW_UP_CAMPAIGN_KEY : INITIAL_CAMPAIGN_KEY;

  const html = buildMarketingEmailShell({
    preheader:
      sequenceStep === 'follow_up'
        ? 'A short follow-up on how NXT1 can help your program operate with more clarity, consistency, and speed.'
        : 'An introduction to how NXT1 helps programs operate like a modern digital athletic department.',
    eyebrow: sequenceStep === 'follow_up' ? 'Follow-Up' : 'NXT1 Sports',
    title:
      sequenceStep === 'follow_up'
        ? 'A Better Operating Standard For Programs'
        : 'An Invite For Your Team',
    subtitle:
      sequenceStep === 'follow_up'
        ? 'A short, practical follow-up and an invitation to see the platform in action.'
      ...
      ${
        sequenceStep === 'follow_up'
          ? `<p style="margin:0 0 16px 0;font-size:19px;line-height:1.65;color:#1f2937;">
              I wanted to follow up on my earlier note in case the timing is better now.
            </p>
            <p style="margin:0;font-size:19px;line-height:1.65;color:#1f2937;">
              NXT1 is designed to help organizations like ${organizationLabel} run with more structure, better delegation, and a more modern operating layer across communication, media, workflows, and day-to-day execution.
            </p>`
          : `<p style="margin:0 0 16px 0;font-size:19px;line-height:1.65;color:#1f2937;">
    ...
    sectionsHtml: [
      `
        ...
        <p style="margin:0 0 14px 0;font-size:18px;line-height:1.65;color:#1f2937;">
          ${
            sequenceStep === 'follow_up'
              ? `The conversation is direct and practical, with a focused demo rather than a broad sales presentation. If this is relevant for you, or better suited for someone else on the ${organizationLabel} team, I would be glad to coordinate it.`
      ...
      `
        ...
        <p style="margin:0 0 14px 0;font-size:18px;line-height:1.65;color:#1f2937;">
          ${
            sequenceStep === 'follow_up'
              ? `If this is relevant for you, I would be glad to send over a few options for a short introduction next week.`
        ...
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;">
          <tr>
            <td style="background-color:rgba(204,255,0,0.10);border:1px solid rgba(204,255,0,0.28);border-left:4px solid #ccff00;border-radius:8px;padding:16px;">
              <p style="margin:0;font-size:18px;line-height:1.65;color:#111827;"><span style="font-weight:700;">${sequenceStep === 'follow_up' ? 'Open to a short conversation?' : 'Brief, direct, and practical.'}</span> ${
                sequenceStep === 'follow_up'
                  ? 'If someone else on your staff is the better contact for this, feel free to point me in the right direction.'
                  : 'We can focus the demo around staff coordination, communication flow, and the operational priorities that matter most to your program.'
              }</p>
            </td>
          </tr>
        </table>
