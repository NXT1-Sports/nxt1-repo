 ...
 * @module @nxt1/backend/services/marketing/campaigns/legacy/legacy-onboarding-completion-email
 *
...
import { sendOutboundMarketingEmail } from '../../email/outbound-email.service.js';
import { logger } from '../../../../utils/logger.js';
import { toAbsoluteAppUrl } from '../../../../utils/app-url.js';
import type { RuntimeEnvironment } from '../../../../config/runtime-environment.js';
import { getLegacyMigrationCreditCents } from './legacy-subscription-migration-recipients.js';
...
function renderTemplate(
  ...
  return template
    ...
    .replaceAll('{{creditAmount}}', escapeHtml(variables.creditAmount))
...
export async function sendLegacyOnboardingCompletionEmail(
  ...
  const creditCents = getLegacyMigrationCreditCents(email) ?? 0;
  ...
  const html = renderTemplate(template, {
    firstName,
    creditAmount: formatUsd(creditCents),
    walletUrl,
    helpCenterUrl,
    supportUrl,
  });
