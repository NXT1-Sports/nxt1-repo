/**
 * @fileoverview Automated Investors & Partnerships outbound lifecycle service
 * @module @nxt1/backend/services/marketing/lifecycle/investors-partnerships-outbound-automation
 */
...
import { readIntegerPropertyByCandidates } from '../integrations/notion/notion-property-helpers.js';
import {
...
import { sendInvestorsPartnershipsEmail } from '../email/campaigns/investors/investors-partnerships-email.service.js';
...
const DAILY_BUDGET_COLLECTION = 'MarketingInvestorsPartnershipOutboundDailyBudget';
const DEFAULT_SEND_LIMIT = 250;
const DEFAULT_DAILY_CAP = 250;
const NOTION_SYNC_LIMIT = 1000;
const MAX_OUTBOUND_BATCH = 250;
...
export interface InvestorsPartnershipsOutboundSendResult {
...
function buildNotionLeadSyncCandidate(page: NotionPage): NotionLeadSyncCandidate | null {
  ...
  if (stage !== 'Lead' && stage !== 'Contacted' && stage !== 'Replied' && stage !== 'Bounced') {
    return null;
  }
  ...
  const touchCount = readIntegerPropertyByCandidates(
...
function parseLeadDoc(id: string, data: Record<string, unknown>): OutboundLeadRecord {
  ...
  return {
    ...
    leadType: (data['leadType'] as LeadType | undefined) ?? 'Strategic Partner',
...
async function reconcileBouncedDispatches(
  ...
  const bouncedDispatches = await MarketingEmailDispatchModel.find({
    environment,
    sendStatus: 'bounced',
    'metadata.outboundLeadKind': 'investors_partnerships',
  ...
  for (const dispatch of bouncedDispatches) {
    ...
    try {
      ...
      logger.error('[InvestorsOutbound] Failed to sync delayed bounced lead to Notion', {
        leadId,
...
export async function reconcileAllPendingInvestorsPartnershipsBouncedDispatches(input: {
  readonly db: FirebaseFirestore.Firestore;
  readonly environment: RuntimeEnvironment;
...
export async function syncPendingInvestorsPartnershipsDeadLetterLeadsToNotion(input: {
  readonly db: FirebaseFirestore.Firestore;
  readonly environment: RuntimeEnvironment;
  ...
  for (const doc of snapshot.docs) {
    ...
    try {
      ...
      failed += 1;
      logger.error('[InvestorsOutbound] Failed to sync dead-letter lead to Notion', {
        leadId: record.id,
        email: record.email,
        error: error instanceof Error ? error.message : String(error),
      });
...
async function markLeadFailure(
  ...
  if (isBounce) {
    try {
      ...
      logger.info('[InvestorsOutbound] Synced bounced lead to Notion', {
        leadId: record.id,
        email: record.email,
        notionStatus: notionResult.status,
      });
    } catch (notionError) {
      logger.error('[InvestorsOutbound] Failed to sync bounced lead to Notion', {
        leadId: record.id,
        email: record.email,
...
export async function runInvestorsPartnershipsOutboundInitialSend(
  input: SendInput
): Promise<InvestorsPartnershipsOutboundSendResult> {
  const sendLimit = getLimit(input.limit, DEFAULT_SEND_LIMIT, MAX_OUTBOUND_BATCH);
  const dailyCap = getLimit(input.dailyCap, DEFAULT_DAILY_CAP, MAX_OUTBOUND_BATCH);
...
export async function runInvestorsPartnershipsOutboundFollowUpSend(
  input: SendInput
): Promise<InvestorsPartnershipsOutboundSendResult> {
  const sendLimit = getLimit(input.limit, DEFAULT_SEND_LIMIT, MAX_OUTBOUND_BATCH);
  const dailyCap = getLimit(input.dailyCap, DEFAULT_DAILY_CAP, MAX_OUTBOUND_BATCH);
...
export const __investorsPartnershipsOutboundAutomationTestUtils = {
