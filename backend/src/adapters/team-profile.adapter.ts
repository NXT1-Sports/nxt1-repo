/**
 * @fileoverview Team Profile Adapter
 * @module @nxt1/backend/adapters/team-profile
 *
...
function generateQuickStats(
  ...
  return {
    pageViews: teamCode.totalTraffic ?? 0,
  ...
  if (orgId && db) {
    try {
      for (const collectionName of ORGANIZATION_COLLECTIONS) {
        const orgSnap = await db.collection(collectionName).doc(orgId).get();
        if (!orgSnap.exists) {
          continue;
        }
