/**
 * @fileoverview Unit Tests — ExportService (CSV, PDF, XLSX & PPTX Generation)
...
function pptxOpts(overrides?: Partial<PptxExportOptions>): PptxExportOptions {
  return {
    title: 'Top Prospects Deck',
    description: 'Generated by Agent X for staff review.',
    columns: [...sampleColumns],
    rows: [...sampleRows],
    ...overrides,
...
describe('ExportService', () => {
  ...
  describe('generatePptx', () => {
    it('uses Gamma when enabled for a structured text deck', async () => {
      ...
      await expect(service.generatePptx(pptxOpts())).resolves.toEqual(gammaPptx);
      expect(generatePptx).toHaveBeenCalledWith(
    ...
    it('falls back to the local renderer when Gamma generation fails', async () => {
      const generatePptx = vi.fn().mockRejectedValue(new Error('Gamma unavailable'));
      const generatePdf = vi.fn();
      service = new ExportService({ isEnabled: true, generatePptx, generatePdf });

      const result = await service.generatePptx(pptxOpts());
    ...
    it('passes brand and image-specific decks through Gamma when enabled', async () => {
      const gammaPptx = Buffer.from('PK\x03\x04gamma-branded-pptx');
      const generatePptx = vi.fn().mockResolvedValue(gammaPptx);
      const generatePdf = vi.fn();
      service = new ExportService({ isEnabled: true, generatePptx, generatePdf });
      ...
      const result = await service.generatePptx(
        pptxOpts({
          brandPrimaryColor: '#0055AA',
          brandSecondaryColor: '#D9A441',
          brandBackgroundMode: 'balanced',
          imageUrls: [TINY_PNG_DATA_URL],
        })
      );
      ...
      expect(result).toEqual(gammaPptx);
    });
    ...
    it('should generate a deck containing PowerPoint slide package entries', async () => {
      const result = await service.generatePptx(
        pptxOpts({
          sections: multiSectionContent(),
        })
      ...
      const binary = result.toString('binary');
      expect(binary).toContain('ppt/presentation.xml');
      expect(binary).toContain('ppt/slides/slide1.xml');
      expect(binary).toContain('ppt/slides/slide2.xml');
    });

    it('should embed images in generated decks', async () => {
      const result = await service.generatePptx(
        pptxOpts({
          imageUrls: [TINY_PNG_DATA_URL],
          sections: [
            {
              ...
              bodyParagraphs: ['Use this slide for the overview.'],
              imageUrls: [TINY_PNG_DATA_URL],
            },
          ],
        })
      ...
      expect(result.toString('binary')).toContain('ppt/media/image-');
    });
  });
  ...
  describe('generatePdf', () => {
      ...
      await expect(
        service.generatePdf(
          pdfOpts({
            ...
            columns: undefined,
            rows: undefined,
            bodyParagraphs: ['This summary should render as a branded narrative PDF.'],
          })
        )
      ).resolves.toEqual(gammaPdf);
    ...
    it('uses Gamma for table-backed PDFs when enabled', async () => {
      const gammaPdf = Buffer.from('%PDF-1.7 gamma-table-pdf');
      const generatePptx = vi.fn();
      const generatePdf = vi.fn().mockResolvedValue(gammaPdf);
      service = new ExportService({ isEnabled: true, generatePptx, generatePdf });

      await expect(service.generatePdf(pdfOpts())).resolves.toEqual(gammaPdf);

      expect(generatePdf).toHaveBeenCalledWith(
        expect.objectContaining({
          title: 'Test Report',
          columns: expect.arrayContaining([expect.objectContaining({ label: 'Name' })]),
          rows: expect.arrayContaining([expect.arrayContaining(['John Doe'])]),
        })
      );
    });

    it('uses Gamma presentation format only when explicitly requested', async () => {
