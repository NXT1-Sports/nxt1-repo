 ...
 * @module @nxt1/backend/modules/agent/services/analytics
 *
  ...
  async discover(query: DiscoverAnalyticsTemplatesQuery): Promise<DiscoveredAnalyticsTemplate[]> {
    ...
    return templates.map((template) => {
      const relevanceScore =
        template.templateKey === keywordLower
          ? 100
          : template.templateKey.startsWith(keywordLower)
            ? 80
            : template.aliases.some((a: string) => a === keywordLower)
              ? 75
              : template.aliases.some((a: string) => a.startsWith(keywordLower))
                ? 60
                : 50;
