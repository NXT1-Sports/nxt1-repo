/**
 * @fileoverview Delete Combine Metrics Tool — Hard delete metrics
...
const DeleteCombineMetricsInputSchema = z.object({
  docId: z.string().trim().min(1),
  userId: z.string().trim().min(1),
});

export class DeleteCombineMetricsTool extends BaseTool {
  readonly name = 'delete_combine_metrics';
  readonly description = 'Hard deletes combine metrics.';
  readonly parameters = DeleteCombineMetricsInputSchema;
  ...
  async execute(
    ...
    const parsed = DeleteCombineMetricsInputSchema.safeParse(input);
    ...
    try {
      const accessGrant = await createProfileWriteAccessService(
        ...
        targetUserId: userId,
        action: 'tool:delete_combine_metrics',
      });
    ...
    context?.emitStage?.('submitting_job', { icon: 'database', phase: 'delete_combine_metrics' });
    ...
    try {
      ...
      logger.info('[DeleteCombineMetricsTool] Metrics deleted', { docId, userId });
      return { success: true, data: { docId, userId, deleted: true } };
    } catch (error) {
      logger.error('[DeleteCombineMetricsTool] Failed to delete metrics', {
