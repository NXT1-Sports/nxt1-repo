...
const isUnsupportedLocalMongoRuntime = process.platform === 'darwin' && process.arch === 'arm64';

describe.skipIf(isUnsupportedLocalMongoRuntime)(
  'AgentMutationPolicyService — All Write Tools Integration (real MongoDB)',
  () => {
    ...
    it('create_universal_team_document → 3 steps', async () => {
      ...
      printRows('create_universal_team_document', rows, 3);
      expect(rows).toHaveLength(3);
      expect(rows.map((r) => r.step).sort()).toEqual(['analytics', 'memory', 'sync_delta']);
      expect(rows.every((r) => r.status === 'completed')).toBe(true);
    });
