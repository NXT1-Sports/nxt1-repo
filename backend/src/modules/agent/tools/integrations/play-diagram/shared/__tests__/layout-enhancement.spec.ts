...
function baseLayout(): DiagramLayout {
  return {
    ...
    routes: [
      { from: 'X', label: 'Post', type: 'cut', points: [[70, 295], [70, 250], [95, 230]] },
      { from: 'MLB', label: 'Gap Rush', type: 'go', points: [[280, 262], [280, 220]] },
    ],
  };
}
...
describe('enhanceLayoutForConcept', () => {
  ...
  it('normalizes SL/SR aliases to H/Y when available', () => {
    ...
    layout.routes = [
      ...layout.routes,
      { from: 'SL', label: 'Seam', type: 'go', points: [[160, 295], [160, 180]] },
      { from: 'SR', label: 'Seam', type: 'go', points: [[440, 295], [440, 180]] },
    ];
  ...
  it('defaults football routes to non-smoothed paths except true arc families', () => {
    const layout = baseLayout();
    layout.routes = [
      { from: 'X', label: 'Vert', type: 'go', points: [[70, 295], [70, 180], [70, 110]], curve: true },
      {
        from: 'QB',
        label: 'Wheel',
        type: 'fade',
        points: [[280, 320], [330, 260], [390, 170]],
        curve: false,
      },
    ];
  ...
  it('normalizes suffixed slot aliases like SL1 and SL2 into clean football labels', () => {
    ...
    layout.routes = [
      ...layout.routes,
      { from: 'SL1', label: 'Seam', type: 'go', points: [[160, 295], [160, 180]] },
      { from: 'SL2', label: 'Flat', type: 'cut', points: [[210, 295], [180, 260], [150, 260]] },
    ];
  ...
  it('normalizes existing OL routes to block type in pass protection concepts', () => {
    ...
    layout.routes = [
      ...layout.routes,
      { from: 'LG', label: 'Odd Path', type: 'go', points: [[245, 300], [280, 210]], curve: true },
    ];
