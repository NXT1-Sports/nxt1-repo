...
describe('AskUserTool output selection mode', () => {
  ...
  it('upgrades partial film-report delivery choices to the full checkbox set', async () => {
    const tool = new AskUserTool();
    const input = {
      question: 'How would you like this delivered?',
      category: 'film_review',
      inputMode: 'single_select',
      options: [
      ...
      expect(error.payload.pendingToolCall?.toolInput['steps']).toEqual([
        ...
        expect.objectContaining({
          ...
          options: expect.arrayContaining([
            expect.objectContaining({ id: 'printable_pdf', formatTag: 'PDF' }),
            expect.objectContaining({ id: 'xlsx_workbook', formatTag: 'XLSX' }),
          ]),
        }),
      ]);
    }
  ...
  it('does not append a delivery step when the user already explicitly specified an output format', async () => {
    const tool = new AskUserTool();
    const input = {
      question: 'Confirm film report setup',
      category: 'film_review',
      steps: [
        {
          ...
          inputMode: 'single_select',
          options: [
            { id: 'self_scout', title: 'Self-scout our tendencies' },
            { id: 'opponent_scout', title: 'Opponent scout their tendencies' },
          ],
        },
      ],
      [ASK_USER_CONTEXT_KEY]: {
        agentId: 'performance_coordinator',
        messages: [
          { role: 'user', content: 'Create a printable PDF self scout report from this film' },
        ],
        toolCallId: 'call_ask_user_explicit_pdf',
      },
    };
    ...
    try {
      ...
      expect(isAgentYield(error)).toBe(true);
      if (!isAgentYield(error)) return;
      expect(error.payload.pendingToolCall?.toolInput['steps']).toHaveLength(1);
      expect(error.payload.pendingToolCall?.toolInput['steps']).toEqual([
        expect.objectContaining({
          id: 'report_type',
          inputMode: 'single_select',
  ...
  it('keeps film review picker options instead of rewriting them as scout perspective choices', async () => {
    const tool = new AskUserTool();
    const input = {
      question: 'Which film review should I use for the self-scout report?',
      category: 'film_review',
      inputMode: 'single_select',
      options: [
        {
          id: 'georgetown_full_game_2025',
          title: 'Georgetown Full Game Film (2025)',
          description: '18 tagged plays',
          formatTag: 'CHOICE',
        },
        {
          id: 'week_2_full_game',
          title: 'Week 2 Full Game Film',
          description: '12 tagged plays',
          formatTag: 'CHOICE',
        },
      ],
      [ASK_USER_CONTEXT_KEY]: {
        agentId: 'performance_coordinator',
        messages: [{ role: 'user', content: 'Create film report for self scout from this' }],
        toolCallId: 'call_ask_user_film_picker',
      },
    };
    ...
    try {
      ...
      expect(error.payload.pendingToolCall?.toolInput['options']).not.toEqual(
        expect.arrayContaining([
          expect.objectContaining({ id: 'self_scout' }),
          expect.objectContaining({ id: 'opponent_scout' }),
        ])
      );
    }
  });

  it('preserves caller custom options on callsheet steps while still including delivery format', async () => {
    const tool = new AskUserTool();
    const input = {
      question: 'Callsheet setup',
      category: 'film_review',
      steps: [
        {
          id: 'callsheet_focus',
          question: 'What is the primary focus of this callsheet?',
          inputMode: 'single_select',
          options: [
            { id: 'offense_attack_4_3', title: 'Attack 4-3 Cover 1 Defense' },
            { id: 'red_zone_situational', title: 'Red Zone & Goal Line Focus' },
            { id: '2_minute_tempo', title: '2-Minute Drill & Tempo Calls' },
          ],
        },
      ],
      [ASK_USER_CONTEXT_KEY]: {
        agentId: 'strategy_coordinator',
        messages: [{ role: 'user', content: 'Create me a callsheet from this game film please' }],
        toolCallId: 'call_ask_user_callsheet_focus',
      },
    };
    ...
    try {
      ...
      expect(isAgentYield(error)).toBe(true);
      if (!isAgentYield(error)) return;
      const steps = error.payload.pendingToolCall?.toolInput['steps'] as Array<
        Record<string, unknown>
      >;
      expect(steps).toHaveLength(2);

      // Question 1 must preserve the caller's custom options — NOT rewritten to "Self-scout our tendencies"
      expect(steps[0]).toMatchObject({
        id: 'callsheet_focus',
        prompt: 'What is the primary focus of this callsheet?',
        options: [
          expect.objectContaining({
            id: 'offense_attack_4_3',
            title: 'Attack 4-3 Cover 1 Defense',
          }),
          expect.objectContaining({
            id: 'red_zone_situational',
            title: 'Red Zone & Goal Line Focus',
          }),
          expect.objectContaining({ id: '2_minute_tempo', title: '2-Minute Drill & Tempo Calls' }),
        ],
      });
      expect(JSON.stringify(steps[0])).not.toContain('Self-scout our tendencies');

      // Question 2: The output format step must still be present with all output format options!
      expect(steps[1]).toMatchObject({
        id: 'delivery_format',
        prompt: 'How would you like this delivered?',
        inputMode: 'multi_select',
        options: expect.arrayContaining([
          expect.objectContaining({
            id: 'printable_pdf',
          ...
          expect.objectContaining({ id: 'gamma_deck', title: 'Gamma Deck', formatTag: 'GAMMA' }),
          expect.objectContaining({ id: 'gamma_pdf', title: 'Gamma PDF', formatTag: 'GAMMA' }),
          expect.objectContaining({
            id: 'xlsx_workbook',
            title: 'XLSX Workbook',
            formatTag: 'XLSX',
          }),
          expect.objectContaining({ id: 'csv', title: 'CSV', formatTag: 'CSV' }),
          expect.objectContaining({
            id: 'chat_summary',
            title: 'Chat Summary',
            formatTag: 'CHOICE',
  ...
  it('promotes a top-level ODK callsheet clarification into steps with delivery format', async () => {
    const tool = new AskUserTool();
    const input = {
      question: 'Is the ODK keyed to our team or the opponent?',
      category: 'film_review',
      inputMode: 'single_select',
      options: [
        {
          id: 'our_team',
          title: 'ODK is keyed to our team',
          description: 'D = our defense, O = our offense.',
        },
        {
          id: 'opponent',
          title: 'ODK is keyed to the opponent',
          description: 'D = opponent defense, O = opponent offense.',
        },
      ],
      [ASK_USER_CONTEXT_KEY]: {
        agentId: 'strategy_coordinator',
        messages: [{ role: 'user', content: 'Create me a callsheet from this game please' }],
        toolCallId: 'call_ask_user_callsheet_odk',
      },
    };
    ...
    try {
      ...
      expect(steps).toHaveLength(2);
      expect(steps[0]).toMatchObject({
        prompt: 'Is the ODK keyed to our team or the opponent?',
        inputMode: 'single_select',
        options: [
          expect.objectContaining({ id: 'our_team', title: 'ODK is keyed to our team' }),
          expect.objectContaining({ id: 'opponent', title: 'ODK is keyed to the opponent' }),
        ],
      });
      expect(JSON.stringify(steps[0])).not.toContain('Self-scout our tendencies');
      expect(steps[1]).toMatchObject({
  ...
  it('yields with structured questionnaire steps for program intake and interview requests', async () => {
    const tool = new AskUserTool();
    const input = {
      question: 'Program Intake',
      steps: [
        {
          id: 'focus_area',
          question: 'Which area is your top priority right now?',
          inputMode: 'single_select',
          options: [
            { label: 'Game prep & film breakdown', value: 'film_prep' },
            { label: 'Recruiting & target lists', value: 'recruiting' },
            { label: 'Playbook & practice scripts', value: 'playbook' },
            { label: 'Branding & social graphics', value: 'graphics' },
          ],
        },
        {
          ...
          inputMode: 'text',
          customPlaceholder: 'e.g. staff coordination, recruiting outreach...',
        },
      ],
      [ASK_USER_CONTEXT_KEY]: {
        agentId: 'router',
        messages: [{ role: 'user', content: 'Ask me some questions about my program' }],
        toolCallId: 'call_ask_user_intake',
      },
    };
    ...
    try {
      ...
      const steps = error.payload.pendingToolCall?.toolInput['steps'];
      expect(steps).toHaveLength(2);
      expect(steps).toEqual([
        expect.objectContaining({
          id: 'focus_area',
          prompt: 'Which area is your top priority right now?',
          inputMode: 'single_select',
          options: expect.arrayContaining([
            expect.objectContaining({ id: 'film_prep', title: 'Game prep & film breakdown' }),
            expect.objectContaining({ id: 'recruiting', title: 'Recruiting & target lists' }),
          ]),
        }),
        expect.objectContaining({
          id: 'biggest_challenge',
          prompt: 'What is your biggest operational bottleneck?',
          inputMode: 'text',
          customPlaceholder: 'e.g. staff coordination, recruiting outreach...',
        }),
      ]);
