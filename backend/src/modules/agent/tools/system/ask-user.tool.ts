...
const askUserOptionSchema = z.union([z.string().trim().min(1).max(120), askUserOptionObjectSchema]);
...
function normalizeAskUserOption(
  ...
  const inferredFormatTag = inferFormatTagFromOptionText(
    `${title} ${id} ${option.description ?? ''}`
  );
  const formatTag =
...
function isFilmReportDeliveryStep(step: {
  readonly id?: string;
  readonly question?: string;
  readonly prompt?: string;
}): boolean {
  const text = `${step.id ?? ''} ${step.question ?? step.prompt ?? ''}`.toLowerCase();
  if (isFilmContextPickerStep(step)) return false;
  if (
    /\b(kind|type|perspective|focus)\b.*\breport\b|\breport\b.*\b(kind|type|perspective|focus)\b/.test(
      text
    )
  ) {
    return false;
...
function isFilmContextPickerStep(step: {
  readonly id?: string;
  readonly question?: string;
  readonly prompt?: string;
}): boolean {
  const text = `${step.id ?? ''} ${step.question ?? step.prompt ?? ''}`.toLowerCase();
  return /\b(?:which|what)\s+(?:film|film\s+review|review|video|game|source|clip)\b|\b(?:film|film\s+review|review|video|game|source|clip)s?\s+should\s+i\s+use\b|\bfrom\s+(?:which|what)\s+(?:film|film\s+review|review|video|game|source|clip)\b/.test(
    text
  );
}

function isFilmReportPerspectiveStep(step: {
  readonly id?: string;
  readonly question?: string;
  readonly prompt?: string;
}): boolean {
  const text = `${step.id ?? ''} ${step.question ?? step.prompt ?? ''}`.toLowerCase();
  if (isFilmContextPickerStep(step)) return false;
  if (/\bcall\s*sheet|callsheet|wristband\b/.test(text)) return false;
...
function isFormatOnlyOptionList(
...
function normalizeFilmReportPerspectiveOptions(): readonly ReturnType<
  typeof normalizeAskUserOption
>[] {
...
function normalizeAskUserStep(
  ...
  const forceFilmReportPerspective =
    forceFilmReportDeliveryContext &&
    isFilmReportPerspectiveStep(step) &&
    (!normalizedOptions ||
      normalizedOptions.length === 0 ||
      isFormatOnlyOptionList(normalizedOptions));
  ...
  return {
    ...
    inputMode: forceFilmReportDelivery
      ? 'multi_select'
    ...
    ...(normalizedOptions || forceFilmReportDelivery || forceFilmReportPerspective
      ? {
          options: forceFilmReportDelivery
...
function buildFilmReportDeliveryStep(): ReturnType<typeof normalizeAskUserStep> {
  return {
    id: 'delivery_format',
    prompt: 'How would you like this delivered?',
    inputMode: 'multi_select',
    options: normalizeFilmReportDeliveryOptions(undefined),
    allowCustomOption: true,
  };
}

function isFilmReportExplicitFormatSpecified(messages: readonly LLMMessage[]): boolean {
  for (let i = messages.length - 1; i >= 0; i--) {
    ...
    if (msg.role === 'user') {
      ...
      if (
        /\bchat\s+only\b|\bjust\s+in\s+chat\b|\bchat\s+summary\b|\bno\s+export\b|\bno\s+file\b/.test(
          text
        )
      ) {
        return true;
...
function isFilmReportAskContext(params: {
  ...
  const selectedContextText = (params.selectedContexts ?? [])
    .map(
      (context) =>
        `${context.kind} ${context.title} ${context.summary ?? ''} ${context.source?.type ?? ''}`
    )
    .join(' ');
  ...
  const stepText = (params.steps ?? [])
    .map((step) => `${step.id ?? ''} ${step.question}`)
    .join(' ');
...
export class AskUserTool extends BaseTool {
  ...
  readonly description =
    'Pause execution to wait for the user when you cannot proceed without their input, or to present an interactive questionnaire/intake form when the user asks you to interview them, ask them questions, or collect preferences.\n\n' +
    'CRITICAL — TWO-STEP USAGE:\n' +
    '  1. FIRST: write your actual question or warm intro to the user as a normal conversational chat message (assistant prose). Be warm, specific, and complete — this is what the user reads in chat.\n' +
    ...
    'Example (intake / interview questionnaire):\n' +
    '  Assistant message: "Let\'s dial in your program priorities so I can better assist you."\n' +
    ...
    'INTAKE & QUESTIONNAIRE RULE:\n' +
    '  • When the user asks you to "ask me some questions", "interview me", "onboard me", or gather their preferences/goals, NEVER dump a static list of numbered questions in prose. Instead, write a brief warm intro, then call `ask_user` with structured `steps` so the questions render as an interactive card.\n\n' +
  ...
  async execute(input: Record<string, unknown>): Promise<ToolResult> {
    ...
    const isExplicitFormatGiven = isFilmReportExplicitFormatSpecified(ctx.messages);
    const shouldPromoteTopLevelQuestionToSteps =
      filmReportAskContext &&
      rawNormalizedSteps === undefined &&
      !isExplicitFormatGiven &&
      topLevelQuestionAsStep.inputMode !== 'text' &&
      (parsed.data.options?.length ?? 0) > 0 &&
      !isFilmReportDeliveryStep(topLevelQuestionAsStep) &&
      !isFilmContextPickerStep(topLevelQuestionAsStep);
    const effectiveRawNormalizedSteps = shouldPromoteTopLevelQuestionToSteps
      ? [normalizeAskUserStep(topLevelQuestionAsStep, 0, filmReportAskContext)]
      : rawNormalizedSteps;
    const hasExplicitDeliveryStep =
      effectiveRawNormalizedSteps?.some((step) => isFilmReportDeliveryStep(step)) ?? false;
    const shouldAppendFilmDeliveryStep =
      filmReportAskContext &&
      effectiveRawNormalizedSteps !== undefined &&
      effectiveRawNormalizedSteps.length > 0 &&
      !hasExplicitDeliveryStep &&
      !isExplicitFormatGiven &&
      !isFilmContextPickerStep(topLevelQuestionAsStep);
    const normalizedSteps = shouldAppendFilmDeliveryStep
      ? [...effectiveRawNormalizedSteps, buildFilmReportDeliveryStep()]
      : effectiveRawNormalizedSteps;

    const shouldRenderSelectionCard =
      !!normalizedSteps?.length ||
      (parsed.data.inputMode !== undefined &&
    ...
    const forceTopLevelFilmReportPerspective =
      ...
      (!normalizedTopLevelOptions ||
        normalizedTopLevelOptions.length === 0 ||
        isFormatOnlyOptionList(normalizedTopLevelOptions)) &&
    ...
    const normalizedOptions = forceTopLevelFilmReportDelivery
    ...
    const pendingToolCall = ctx.toolCallId
      ? {
          ...
          toolInput: {
            ...
            ...(shouldRenderSelectionCard &&
            (normalizedOptions || (normalizedSteps && normalizedSteps.length > 0))
              ? {
                  multiSelect:
                    !forceTopLevelFilmReportPerspective &&
