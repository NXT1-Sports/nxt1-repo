...
export class PerformanceCoordinatorAgent extends BaseAgent {
  ...
  getSystemPrompt(_context: AgentSessionContext): string {
    ...
    const prompt = [
      ...
      '   - If an annotated snapshot or `-annotated-crop-` attachment/imageUrl is present, call `analyze_image` on those image URL(s). If both are present, pass both URLs and prioritize the crop for identifying the marked player.',
      '   - If no annotated image attachment/imageUrl is present but `hasDrawing: true`, annotation bounds, draw bounds, or user wording says they circled/drew/marked something, call `ffmpeg_generate_thumbnail` on the clip/video URL BEFORE any video analysis. Use `inputPath` from the clip/video URL, set `time` from the marked-frame `currentTimeSec` / "Marked-frame timestamp" when available, and pass `cropBounds` from the video-frame normalized annotation bounds (`minX`, `minY`, `maxX`, `maxY`). Otherwise use the midpoint of `timeRange`; do not use play `startSec` unless it is the only timestamp available. Use an outputPath like `film-review-marked-frame.jpg`. Then call `analyze_image` on the returned `cropImageUrl` / `imageUrl` with the same bounds and ask it to identify exactly what is inside the cropped selected area.',
      "   - A generated FFmpeg thumbnail is a raw video frame; it will not visibly include the user's circle. When `cropBounds` are supplied, `ffmpeg_generate_thumbnail` returns the cropped selected area as `cropImageUrl` / `imageUrl`; use that crop first. Do not identify the natural center of the full frame unless it is inside those bounds.",
      ...
      '- First generate a still with `ffmpeg_generate_thumbnail` at the marked-frame timestamp/currentTimeSec when available; otherwise use the midpoint of the play window. Always pass annotation `cropBounds` when bounds exist so the tool returns a cropped selected-area image. Then run `analyze_image` on `cropImageUrl`/`imageUrl` using the provided video-frame normalized bounds.',
