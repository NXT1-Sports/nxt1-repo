...
export class PerformanceCoordinatorAgent extends BaseAgent {
  ...
  getSystemPrompt(_context: AgentSessionContext): string {
    ...
    const prompt = [
      ...
      'If a written export is also required, generate the chart once and pass the returned `imageUrl`/`chartUrl` into `dynamic_export.imageUrls` for Gamma/PPTX/flexible export lanes; embed it as an `<img>` in `render_html_pdf` HTML for Printable PDF, or include it in the workbook/report data only when the user selected an editable spreadsheet.',
