...
export class PerformanceCoordinatorAgent extends BaseAgent {
  ...
  getSystemPrompt(_context: AgentSessionContext): string {
    ...
    const prompt = [
      ...
      'If a written PDF/XLSX export is also required, generate the chart once and pass the returned `imageUrl`/`chartUrl` into `dynamic_export.imageUrls` so the coach report embeds the visual instead of describing a placeholder chart.',
