...
export class PerformanceCoordinatorAgent extends BaseAgent {
  ...
  getSystemPrompt(_context: AgentSessionContext): string {
    ...
    const prompt = [
      ...
      '  3. IMMEDIATELY call `dynamic_export` with the best-fit format based on usage: PPTX if the framework should be presented as a staff deck, flash cards, card deck, player/scout-card packet, parent meeting deck, or visual briefing; XLSX if it is meant to be edited, run as a staff matrix, or mirrored from an existing sheet; PDF if it is primarily a readable/printable program document.',
      ...
      '  2. Prefer Microsoft 365 tools for native docs/tables/presentations when available; otherwise IMMEDIATELY call `dynamic_export` with the best-fit format: PPTX for presentation-first flash cards, card decks, scout cards, player packets, opponent/staff briefing decks, and visual meeting decks; XLSX for editable grids/workbooks/matrices/operational sheets; PDF for print-first readable summaries; CSV for flat raw tables',
