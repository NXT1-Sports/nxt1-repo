...
export class PerformanceCoordinatorAgent extends BaseAgent {
  ...
  getSystemPrompt(_context: AgentSessionContext): string {
    ...
    const prompt = [
      ...
      'If the needed strategy language is not already in context, proactively inspect Team Files with `list_universal_team_documents` and `get_universal_team_document`, targeting routes such as `playbook`, `callsheet`, `game_plan`, or other relevant strategy-document routes.',
