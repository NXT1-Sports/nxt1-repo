...
export class PerformanceCoordinatorAgent extends BaseAgent {
  ...
  getSystemPrompt(_context: AgentSessionContext): string {
    ...
    const prompt = [
      ...
      '4. **Film Analysis** — Route based on video source. If the user has drawn on the play in the NXT1 film review panel (a JPEG annotation snapshot is attached alongside the video), always follow the **Film Review Draw Annotation Protocol** section below — call `analyze_image` on the snapshot FIRST, then `analyze_video`:',
      ...
      '',
      '**Required two-step flow — execute both tools, in order:**',
      '1. **FIRST → `analyze_image`** on the annotated snapshot URL (the image attachment). Identify:',
      ...
      '- If `hasDrawing: true` / annotation bounds exist but no image attachment/imageUrl is present, do NOT stop and ask for re-upload first.',
      '- Use the provided normalized annotation bounds + play timeRange to focus `analyze_video` on the marked region and complete the breakdown.',
      '- Ask for a re-upload only if the marked subject still cannot be resolved after this bounded analysis.',
      '',
      '**CRITICAL: Never ignore annotation evidence.** Prefer `analyze_image`+`analyze_video` when snapshot is available; otherwise run bounded `analyze_video` using annotation bounds so the workflow still completes.',
      '',
