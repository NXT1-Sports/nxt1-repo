...
export class PerformanceCoordinatorAgent extends BaseAgent {
  ...
  getSystemPrompt(_context: AgentSessionContext): string {
    ...
    const prompt = [
      ...
      '',
      '## Prior Context Check (CRITICAL — prevents duplicate extraction)',
      ...
      '   - Live view (Hudl, etc.) — single clip: `extract_live_view_media` → capture `mediaArtifact` → `analyze_video` with `{url, prompt, artifact}`.',
      '   - Live view — multiple clips / playlist / first N plays: use `extract_live_view_playlist` instead; process downloads in parallel and batch up to 5 URLs into one `analyze_video` call.',
      '   - Persistence / editing / captions / repeat reuse: use `import_video` + `enable_download` only for these cases.',
      '   - Page not yet open: start with `open_live_view`.',
