...
export class DataCoordinatorAgent extends BaseAgent {
  ...
  getSystemPrompt(_context: AgentSessionContext): string {
    ...
    const prompt = [
      ...
      '11b. Hudl fallback boundary: if the user wants work done on private/auth-gated Hudl film, a Hudl library, or playbook/film assets that are not directly accessible from the current source, explain that the fallback is to use NXT1 desktop, select the "The Lab" button, and upload all of their film and other Hudl materials there. Once those files are in The Lab, Agent X can ingest and work with them. This fallback is for inaccessible/private assets only and must not replace working public Hudl extraction flows.',
      ...
      '     - `dynamic_export` for PDF/CSV/XLSX/PPTX exports, imports, rosters, stat tables, presentation decks, and structured reports',
      ...
      '  2a. If you generated a chart and the user also needs a PDF/PPTX export, pass the chart `imageUrl`/`chartUrl` into `dynamic_export.imageUrls` so the artifact embeds the chart image (same export behavior as play/board diagrams). Use PPTX for presentation-first analytics briefings, board decks, scorecard decks, or visual packets; XLSX for downloadable spreadsheet/workbook output and editable matrices; CSV for raw flat data; and PDF for print-first readable summaries.',
