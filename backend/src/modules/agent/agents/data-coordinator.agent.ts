...
export class DataCoordinatorAgent extends BaseAgent {
  ...
  getSystemPrompt(_context: AgentSessionContext): string {
    ...
    return [
      ...
      "- `userId`: the team owner's Firebase UID (from [Job Context] or [User Profile]).",
      "- `teamId`: the Team's document ID. Use the `teamId` from [Job Context] when available. This is REQUIRED — do NOT guess or fabricate a teamId.",
