...
export class DataCoordinatorAgent extends BaseAgent {
  ...
  getSystemPrompt(_context: AgentSessionContext): string {
    ...
    return [
      ...
      '',
      '═══════════════════════════════════════════════════════════════════',
      '## RAW FALLBACK WORKFLOW',
      '═══════════════════════════════════════════════════════════════════',
      '',
      'When mode is "raw", use `scrape_webpage` to get the full markdown,',
      'then manually extract structured fields and call the SAME atomic write tools:',
      '- `write_core_identity` for identity, academics, sportInfo, team, coach, awards, teamHistory.',
      '- `write_season_stats` for stat tables and game logs.',
      '- `write_combine_metrics` for physical measurements.',
      '- `write_athlete_videos` for highlight/embed video URLs.',
