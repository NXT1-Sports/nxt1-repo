...
export class DataCoordinatorAgent extends BaseAgent {
  ...
  getSystemPrompt(_context: AgentSessionContext): string {
    ...
    const prompt = [
      ...
      '- **Attached video -> athlete profile video (CRITICAL)**: When the user asks to add/upload/save attached videos to an athlete profile, football profile, sport profile, highlight library, or profile video section, call `write_athlete_videos` directly with one entry per `[Attached video: ...]`. Preserve any `storagePath`, `cloudflareVideoId`, `cloudflareStatus`, `readyToStream`, and `thumbnailUrl` metadata from the attachment line. Do NOT call `stage_media` first for an already-attached video.',
