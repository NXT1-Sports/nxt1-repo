...
export class AdminCoordinatorAgent extends BaseAgent {
  ...
  getSystemPrompt(_context: AgentSessionContext): string {
    ...
    const prompt = [
      ...
      '**RULE: Structured Compliance Reports First → Export → Chat Summary**',
      '',
      'When a user requests ANY of the following, use Microsoft 365 FIRST when Microsoft is connected and the output should live in a native document, spreadsheet, or presentation; otherwise choose the best-fit artifact lane and then reference it in chat:',
      '- Recruiting calendar summaries, contact window timelines, dead period schedules',
      '- Compliance checklists, eligibility verification reports, NCAA bylaw references',
      '- Visit schedule tracking, official/unofficial visit limits and documentation',
      '- Transfer portal timeline tracking, portal window calendars',
      '- Recurring task schedules, automation tracking reports',
      '- NIL disclosure checklists, compliance frameworks',
      '- Anything structured (timelines, checklists, calendars, schedules)',
      '',
      'EXECUTION FLOW:',
      '  1. Generate compliance content (calendar, checklist, verification)',
      '  2. Choose the artifact lane by output shape:',
      '     - `render_html_pdf` first for one-page or fixed-layout printable PDFs (calendar one-pagers, checklist sheets, timeline posters, share-ready reference pages)',
      '     - `execute_python_code` first for editable XLS/XLSX workbooks, tracking sheets, and spreadsheet-style matrices built with Python/openpyxl in E2B',
      '     - `dynamic_export` for PPTX decks, Gamma-style multi-section reports, CSV exports, and as the fallback path for PDF/XLSX when the dedicated route is not appropriate',
      ...
      '  4. Never paste large content blocks (calendars, checklists) directly in chat',
      '',
