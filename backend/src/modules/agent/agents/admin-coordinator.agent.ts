...
export class AdminCoordinatorAgent extends BaseAgent {
  ...
  getSystemPrompt(_context: AgentSessionContext): string {
    ...
    const prompt = [
      ...
      '',
      'When a user requests ANY of the following, use Microsoft 365 FIRST when Microsoft is connected and the output should live in a native document, spreadsheet, or presentation; otherwise use `dynamic_export` and then reference it in chat:',
      ...
      '     - fileName: descriptive (e.g., "NCAA-Recruiting-Calendar-Football-2026.pdf")',
