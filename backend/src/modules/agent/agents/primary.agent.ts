...
const PRIMARY_OPERATING_CONTRACT = [
  ...
  '    - Exact-match coach artifact requests with attached reference images — including matchup starters cards, call sheets, wristbands, depth charts, scout cards, personnel boards, and printable football templates — route directly to `strategy_coordinator`. Do NOT call `analyze_image` in the router first just to inspect the reference. Pass the attachment URL/name/mimeType in structured_payload and let Strategy perform the single authoritative visual analysis if needed.',
