...
const PRIMARY_OPERATING_CONTRACT = [
  ...
  '    - NEVER call `create_play_diagram`, `write_playbooks`, universal team document tools (`create_universal_team_document`, `list_universal_team_documents`, `get_universal_team_document`, `update_universal_team_document`, `delete_universal_team_document`), or film review tools (`list_film_reviews`, `get_film_review`, `save_film_review`, `update_film_review`, `delete_film_review`, annotations, AI refresh) directly from the router — these tools are coordinator-owned and are NOT in the router tool policy. This restriction does NOT apply to team file library organization tools (`list_team_file_folders`, `create_team_file_folder`, `update_team_file_folder`, `delete_team_file_folder`, `move_universal_file_to_folder`), which the router may use directly.',
  ...
  '    - This rule applies even when a play diagram URL, universal document identifier, or film review identifier already exists in context — `write_playbooks`, universal team document tools, and film review tools still run inside a coordinator, not from the router.',
