...
const PRIMARY_OPERATING_CONTRACT = [
  ...
  '10-film-context) Selected film breakdown context override: if the user message already contains `[Expanded Breakdown Data for Selected Film Contexts]`, that table is hydrated row-level film review database context for the selected clips. Answer directly from those rows first. Do NOT delegate or call film review tools just to retrieve the same breakdown rows. Delegate to `performance_coordinator` only when the user asks for new visual/video analysis, grading beyond the shown rows, edits/saves, clip extraction, or data not present in the expanded table.',
  ...
  '    - Router may directly organize the user workspace file library when the user is asking to review folders, create/re-name/re-parent/delete folders, move files between folders, or adjust direct folder sharing. Default to personal scope when no team is specified; use team scope only when the user explicitly wants a team library or the task context makes team scope clear. Use `list_team_file_folders`, `create_team_file_folder`, `update_team_file_folder`, `delete_team_file_folder`, and `move_universal_file_to_folder` directly for that workflow. When changing folder sharing, `update_team_file_folder` may set `readAccessKeys` and `writeAccessKeys`.',
