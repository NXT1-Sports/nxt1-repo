...
describe('Agent tool exposure regressions', () => {
  ...
  it('keeps system-auto-included core/research tools out of per-coordinator policy', () => {
    ...
    expect(new AdminCoordinatorAgent().getAvailableTools()).toEqual(
      expect.arrayContaining(['render_html_pdf', 'execute_python_code'])
    );
  });
  ...
  it('exposes Intel persistence to the performance coordinator', () => {
    ...
    expect(agent.getAvailableTools()).toContain('import_film_review_breakdown');
  ...
  it('teaches recruiting coordinator to use database-first research before web fallback', () => {
    ...
    expect(prompt).toContain(
      'Treat "clearest deliverable" and "build the target list" as no explicit format'
    );
  });
  ...
  it('keeps sandbox analysis exposed only on intended agent policies', () => {
    ...
    expect(new DataCoordinatorAgent().getAvailableTools()).toContain('execute_python_code');
    expect(new DataCoordinatorAgent().getAvailableTools()).toContain('render_html_pdf');
    expect(new PerformanceCoordinatorAgent().getAvailableTools()).toContain(
      'execute_sandbox_script'
    );
    expect(new PerformanceCoordinatorAgent().getAvailableTools()).toContain('execute_python_code');
    expect(new PerformanceCoordinatorAgent().getAvailableTools()).toContain('render_html_pdf');
    expect(new StrategyCoordinatorAgent().getAvailableTools()).toContain('execute_sandbox_script');
    expect(new StrategyCoordinatorAgent().getAvailableTools()).toContain('execute_python_code');
    expect(new StrategyCoordinatorAgent().getAvailableTools()).toContain('render_html_pdf');
    expect(new RecruitingCoordinatorAgent().getAvailableTools()).not.toContain(
      'execute_sandbox_script'
    );
    expect(new RecruitingCoordinatorAgent().getAvailableTools()).toContain('execute_python_code');
    expect(new RecruitingCoordinatorAgent().getAvailableTools()).toContain('render_html_pdf');
    expect(new BrandCoordinatorAgent().getAvailableTools()).not.toContain('execute_sandbox_script');
    expect(new BrandCoordinatorAgent().getAvailableTools()).not.toContain('execute_python_code');
    expect(new BrandCoordinatorAgent().getAvailableTools()).toContain('render_html_pdf');
    expect(new AdminCoordinatorAgent().getAvailableTools()).not.toContain('execute_sandbox_script');
    expect(new AdminCoordinatorAgent().getAvailableTools()).toContain('execute_python_code');
    expect(new AdminCoordinatorAgent().getAvailableTools()).toContain('render_html_pdf');
    expect(getRouterToolPolicy()).toContain('execute_sandbox_script');
    expect(getRouterToolPolicy()).toContain('execute_python_code');
  });
  ...
  it('teaches strategy coordinator to use real film analysis for video requests', () => {
    ...
    expect(prompt).toContain(
      'For XLSX/spreadsheet artifacts generated by `execute_python_code`, do NOT call `dynamic_export` again'
    );
    expect(prompt).toContain('NON-NEGOTIABLE WRISTBAND RULE');
    expect(prompt).toContain(
      'Do NOT use `execute_python_code` or `dynamic_export` for the primary artifact unless the user explicitly asks for an editable spreadsheet/source sheet or workbook companion.'
    );
    expect(prompt).toContain(
      'scout team play cards, scout-team look cards, and scout-period cards are printable practice PDFs'
    );
    expect(prompt).toContain(
      'scout team play/look cards are printable practice cards, not presentation card decks'
    );
  ...
  it('exposes live-view extraction tools in the effective runtime policy for film coordinators', () => {
    ...
    expect(performanceTools).toContain('import_film_review_breakdown');
    ...
    expect(strategyTools).toContain('import_film_review_breakdown');
