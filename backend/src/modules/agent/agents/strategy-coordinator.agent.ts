...
export class StrategyCoordinatorAgent extends BaseAgent {
  ...
  getSystemPrompt(context: AgentSessionContext): string {
    ...
    const prompt = [
      ...
      '11. **Play Diagrams** — When a coach or athlete asks to "draw a play", "diagram a route tree", "create a formation diagram", "build a playbook", "create a game plan", "design plays", or any request that results in a multi-play offensive or defensive playbook, call `create_play_diagram` for EACH individual play or scheme as you design it. Do NOT wait until all plays are written in prose and then skip diagrams. Call `create_play_diagram` one time per play, passing a detailed `description` of the formation, routes, and assignments for that specific play, and the known `sport`. The returned `diagramUrl` must be embedded in your message next to each play. NEVER substitute a single `generate_graphic` call as a replacement for individual `create_play_diagram` calls — they serve different purposes: `create_play_diagram` draws X-and-O route diagrams for specific plays, while `generate_graphic` creates a poster/overview. Do NOT say "I have created your diagrams" or "play diagrams are ready" unless you have received a `diagramUrl` from `create_play_diagram` for each play in the same response sequence. Also use `write_playbooks` to persist plays with their `diagramUrl` values to the user\'s team playbook collection.',
      '11a. **Diagram Exports (PDF/CSV)** — When generating a PDF that should include diagrams/images, pass all diagram URLs into `dynamic_export` using `imageUrls` (array). Do not rely on prose-only placeholders like "DIAGRAM:" lines. If a drill/play has a `diagramUrl`, include it in `imageUrls` so the exported PDF embeds the actual images.',
