...
export class StrategyCoordinatorAgent extends BaseAgent {
  ...
  getSystemPrompt(context: AgentSessionContext): string {
    ...
    const prompt = [
      ...
      'For plays, playbooks, callsheets, practice scripts, game plans, and diagrams: treat the first output as a draft/proposal unless the user explicitly asked to save it now.',
      ...
      'Only call persistence tools (`save_gameplan`, `write_playbooks`, `write_callsheet`, `update_callsheet`, `delete_callsheet`, `write_practice_script`, `update_practice_script`, `delete_practice_script`, film review write tools such as `save_film_review`, `update_film_review`, `delete_film_review`, `add_film_review_annotation`, `delete_film_review_annotation`, `refresh_film_review_ai`, `create_play_diagram`, `create_board_diagram`, `dynamic_export`, `write_intel`, `send_email`, `batch_send_email`) AFTER the user confirms with "yes", "go ahead", "do it", "looks good", "build it", or equivalent affirmation.',
      ...
      '  • The task is a revision/update to an artifact the user already approved in the same session, EXCEPT playbooks/plays/game plans/diagrams still require a fresh explicit save/apply request before persisting the revision',
      ...
      '- If no annotated snapshot image is present but drawing metadata/bounds are present, do not block. Run bounded `analyze_video` using annotation bounds + timeRange and complete the breakdown.',
      '- Ask for a re-upload only if the marked subject still cannot be resolved after bounded video analysis.',
      modeHint,
    ]
      .filter(Boolean)
      .join('\n');
