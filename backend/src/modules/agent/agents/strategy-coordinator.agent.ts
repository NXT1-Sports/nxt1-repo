...
export class StrategyCoordinatorAgent extends BaseAgent {
  ...
  getSystemPrompt(context: AgentSessionContext): string {
    ...
    const prompt = [
      ...
      '    • For exact-match or fixed-geometry PDF requests, gather the sample/reference layout and page target, then use `render_html_pdf` with complete HTML/CSS. Use `dynamic_export` only for flexible reports/decks where exact placement is not required.',
      '',
      ...
      '     - `execute_python_code` by default for XLSX, Excel, spreadsheets, workbooks, trackers, matrices, schedules, callsheets, budgets, dashboard-style sheets, and editable grid deliverables. Use Python/openpyxl for professional multi-sheet files with formulas, frozen panes, formatting, charts, and files written to `/home/user/outputs`.',
      '     - `dynamic_export` for PDFs/CSVs/PPTX decks, plans, tables, checklists, calendars, scout-card packets, readable documents, and XLSX fallback only when `execute_python_code` is unavailable or the user explicitly asks for a quick/simple template export. Never call `dynamic_export` after `execute_python_code` already produced the requested XLSX.',
      ...
      '     - Choose export format by workflow: PPTX for presentation-first staff decks, flash cards, card decks, player/scout-card packets, opponent briefing decks, recruiting pitch decks, parent/staff meeting decks, and visual slide packets; `execute_python_code`/XLSX for editable grids, staff boards, matrices, sample-matching sheets, callsheets, practice matrices, call menus, install sheets, and operational documents; `render_html_pdf` for wristband inserts, physical quick-reference cards, and other fixed-geometry printable one-pagers; PDF for print-first/share-first readable summaries only; CSV for raw flat data only',
      ...
      '  - If the requested deliverable is table-like, matrix-like, board-like, menu-like, or checklist-like, assume the coach wants a spreadsheet-style sheet first unless they explicitly ask for PDF, PPTX, or prose-document output.',
      '  - For wristband inserts and other physical quick-reference cards, assume the coach wants a fixed-size printable artifact first. Route those to `render_html_pdf` unless they explicitly ask for an editable spreadsheet/source sheet.',
      ...
      '  - Do NOT assume PDF is the primary output for coach operating documents. Callsheets, practice scripts, install sheets, scouting grids, and similar coaching sheets should default to `execute_python_code`/XLSX or the native saved Files document path unless the user explicitly asks for PDF, print-ready output, or share-first delivery. When the user does explicitly ask for PDF/printable/share-ready output, that instruction overrides the document/XLSX default and you should produce the PDF in the same run rather than leaving them with only a saved document. Wristband inserts are the exception: they should default to `render_html_pdf` because the physical card geometry matters more than workbook editability.',
      '  - Do NOT force flash cards, card decks, scout cards, player-card packets, opponent briefing decks, recruiting decks, or meeting decks into PDF/XLSX. If coaches need to present it slide-by-slide, flip through it as cards, share it as cards, or walk through it in a room, use PPTX unless Microsoft 365 native PowerPoint is connected and explicitly preferred.',
