...
export class StrategyCoordinatorAgent extends BaseAgent {
  ...
  getSystemPrompt(context: AgentSessionContext): string {
    ...
    const prompt = [
      ...
      "  - Use `save_film_review` only when creating a team film review from a known video/media URL; use `update_film_review` for metadata, clips, AI notes, tags, or whole-review timeline row changes; use `list_film_review_sources` to inspect multi-source reviews; use `add_film_review_source` / `update_film_review_source` / `delete_film_review_source` for first-class source-video CRUD; use `get_film_review_source_breakdown` to inspect one source clip's table rows, `update_film_review_source_breakdown` to create/replace/append one source clip's breakdown rows, and `delete_film_review_source_breakdown` to remove stale source-scoped rows; use `extract_film_review_clips` when the user wants selected source clips from a batch clip review turned into their own separate or combined film review cutup(s); use annotation tools for timestamped coaching notes. Prefer the source-breakdown CRUD tools over a generic `update_film_review` call whenever the user is editing only one source clip's game-breakdown rows. For batch clip sessions, never claim the film-review breakdown table was updated unless you actually call `update_film_review_source_breakdown` or `update_film_review` with explicit `timeline` rows tied to the analyzed clips using `sourceId`. If you only have prose analysis or partial failed clips, say you saved notes/summary only and that the table may still be unchanged. Do not create a universal document as the primary output for cutups, source extraction, or film-review CRUD; use a document only for accompanying written reports/notes when requested.",
