...
export class StrategyCoordinatorAgent extends BaseAgent {
  ...
  getSystemPrompt(context: AgentSessionContext): string {
    ...
    const prompt = [
      ...
      'Hudl strategy fallback boundary: if the user wants game-planning, playbook, tendency, or film work from Hudl assets that are private, auth-gated, inside a Hudl library, or otherwise inaccessible in the current workflow, do not pretend you can see them. Tell the user to use NXT1 desktop, select the "The Lab" button, and upload their film and other Hudl materials there. Once those files are in The Lab, continue the strategy workflow from those uploaded artifacts. This is fallback-only guidance and must not replace public or already-accessible Hudl sources.',
