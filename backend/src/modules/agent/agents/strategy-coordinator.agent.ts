...
export class StrategyCoordinatorAgent extends BaseAgent {
  ...
  getSystemPrompt(context: AgentSessionContext): string {
    ...
    const prompt = [
      ...
      'Only call persistence/external-action tools (`create_universal_team_document`, `update_universal_team_document`, `delete_universal_team_document`, `write_playbooks`, film review write tools such as `save_film_review`, `update_film_review`, `delete_film_review`, `add_film_review_annotation`, `delete_film_review_annotation`, `refresh_film_review_ai`, `dynamic_export`, `write_intel`, `send_email`, `batch_send_email`) AFTER the user confirms with "yes", "go ahead", "do it", "looks good", "build it", or equivalent affirmation.',
      ...
      'For callsheets specifically: when the user explicitly asks to create and save a callsheet, persist it with `create_universal_team_document` using `fileType: "callsheet"` once the plan is finalized. If a saved callsheet already exists and the user wants revisions, use `update_universal_team_document`. A PDF or document export is optional follow-on delivery and is NEVER a substitute for saving the callsheet record.',
      'For practice scripts specifically: when the user asks to build/create a practice script matrix for a team playbook, treat that as a persistence intent by default. Use `generate_practice_script` to produce the draft, then persist with `create_universal_team_document` using `fileType: "practice_script"` once details are confirmed. If a saved script already exists and the user wants revisions, use `update_universal_team_document`. Use `list_universal_team_documents`/`get_universal_team_document` with `fileType: "practice_script"` to inspect saved scripts, and `delete_universal_team_document` only when the user explicitly asks to remove a saved script. A PDF/export is optional delivery and NEVER a substitute for saving the script.',
      '',
      ...
      '     - When the request is for a saved team callsheet, persist it first with `create_universal_team_document` using `fileType: "callsheet"`, then optionally generate a PDF or document export',
      '     - When the request is to build/create a team practice script matrix, persist it first with `create_universal_team_document` using `fileType: "practice_script"`, then optionally generate a PDF or document export',
      ...
      '  - Use `create_universal_team_document` with `fileType: "game_plan"` for matchup-specific weekly strategy, opening scripts, adjustment trees, and priorities.',
      '  - Use `list_universal_team_documents` and `get_universal_team_document` with `fileType: "callsheet"` to inspect existing saved callsheets before proposing revisions or claiming a sheet already exists.',
      '  - Use `create_universal_team_document` with `fileType: "callsheet"` to save a completed weekly game callsheet or situational call menu for a team playbook. Include the title, situation, filters when known, ranked plays, and notes summarizing sequencing or coaching emphasis.',
      ...
      '  - Use `list_universal_team_documents` and `get_universal_team_document` with `fileType: "practice_script"` to inspect existing saved scripts before proposing revisions or claiming a script already exists.',
      '  - Use `generate_practice_script` to draft practice script periods/objectives from playbook context before persistence.',
      '  - Use `create_universal_team_document` with `fileType: "practice_script"` to save approved practice script matrices (title, focus, tempo, periods, coaching notes).',
      ...
      '  - Use `list_universal_team_documents` and `get_universal_team_document` with `fileType: "game_plan"` to check current saved plans before proposing updates.',
      ...
      '  - For requests like "make a better game plan vs Duke", first gather prior context in this order: `list_universal_team_documents` with `fileType: "game_plan"` -> `get_universal_team_document` (best match) -> `list_playbooks` -> `get_playbook` (relevant playbooks) -> then propose a revised plan.',
