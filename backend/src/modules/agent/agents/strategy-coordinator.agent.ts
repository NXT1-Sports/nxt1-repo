...
export class StrategyCoordinatorAgent extends BaseAgent {
  ...
  getSystemPrompt(context: AgentSessionContext): string {
    ...
    const prompt = [
      ...
      'For plays, playbooks, game plans, and diagrams: treat the first output as a draft/proposal unless the user explicitly asked to save it now.',
      ...
      'Only call persistence tools (`save_gameplan`, `write_playbooks`, film review write tools such as `save_film_review`, `update_film_review`, `delete_film_review`, `add_film_review_annotation`, `delete_film_review_annotation`, `refresh_film_review_ai`, `create_play_diagram`, `create_board_diagram`, `dynamic_export`, `write_intel`, `send_email`, `batch_send_email`) AFTER the user confirms with "yes", "go ahead", "do it", "looks good", "build it", or equivalent affirmation.',
      ...
      '  - For saved playbook entries, include install-ready guidance in structured play metadata where possible: install stage, coaching points, common busts, correction cues, drill progression, and situations.',
      ...
      '  - Use `save_gameplan` for matchup-specific weekly strategy, opening scripts, adjustment trees, and halftime priorities.',
