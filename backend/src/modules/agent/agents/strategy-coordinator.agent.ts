...
export class StrategyCoordinatorAgent extends BaseAgent {
  ...
  getSystemPrompt(context: AgentSessionContext): string {
    ...
    const prompt = [
      ...
      '     - Choose export format by workflow: PPTX for presentation-first staff decks, flash cards, card decks, player/scout-card packets, opponent briefing decks, recruiting pitch decks, parent/staff meeting decks, and visual slide packets; XLSX for editable grids, staff boards, matrices, sample-matching sheets, callsheets, practice matrices, call menus, install sheets, wristband sheets, quick-reference boards, and operational documents; PDF for print-first/share-first readable summaries only; CSV for raw flat data only',
      ...
      '  - If the requested deliverable is table-like, matrix-like, board-like, menu-like, checklist-like, or a wristband/quick-reference artifact, assume the coach wants a spreadsheet-style sheet first unless they explicitly ask for PDF, PPTX, or prose-document output.',
      ...
      '  - Treat styling as part of the coaching artifact. Infer a clean, request-specific look from the sport, team/program context, and artifact type; use team/organization colors when available, and use section.themeColor, concise headings, short labels, and logical grouping so the board/sheet is easy to scan on game day.',
      ...
      '  - When branding fields are found, pass `organizationName`, `brandPrimaryColor`, `brandSecondaryColor`, and `logoUrl` into `dynamic_export` automatically. For PPTX/slide-style exports, also pass `brandBackgroundMode: "balanced"` unless the user explicitly asks for a more minimal or bolder treatment. Do not make the user repeat branding already present in platform data.',
