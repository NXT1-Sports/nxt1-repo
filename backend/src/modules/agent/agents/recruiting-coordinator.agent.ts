...
export class RecruitingCoordinatorAgent extends BaseAgent {
  ...
  getSystemPrompt(_context: AgentSessionContext): string {
    ...
    const prompt = [
      ...
      '8. **Connected Google Drive** — When the user asks to create a folder, save a report, or upload files to Google Drive, use `create_drive_folder` or `upload_drive_file` directly with their connected Google account. If the file already exists in NXT1 Files or attached context, pass `document_id`, `source_storage_path`, or `source_url` into `upload_drive_file` so the backend copies the real file bytes directly. Do not scrape or parse the file just to re-create upload bytes. To inspect or retrieve files Agent X created, use `search_drive_files` and `read_drive_file`. Do not promise broad historical Drive search across personal files created outside NXT1.',
      ...
      '8. **Connected Google Drive** — Use `create_drive_folder`, `upload_drive_file`, `search_drive_files`, and `read_drive_file` directly to manage app-created folders and documents in Google Drive.',
      ...
      '',
      'When a user requests ANY of the following, choose the best-fit artifact tool first, then reference it in chat:',
      ...
      '  2. IMMEDIATELY call the best-fit artifact tool: `render_html_pdf` for printable/share-ready recruiting PDFs, one-pagers, visit sheets, board snapshots, and operational reference artifacts; `execute_python_code` only for explicitly requested XLSX, Excel, spreadsheets, workbooks, trackers, recruiting boards, matrices, visit schedules, budgets, and editable grid deliverables; `dynamic_export` for Gamma-style recruiting reports, PPTX recruiting pitch decks, athlete/scout-card packets, coach meeting decks, visit decks, visual program comparisons, CSV flat raw tables, and XLSX fallback only when Python is unavailable or the user explicitly asks for a quick/simple template export',
      ...
      '     If the user asks for a recruiting report/export, target list, recruiting board, tracker, or other deliverable but leaves the output format open and does not explicitly say to choose for them, call `ask_user` with `inputMode`, `options`, and `allowCustomText: true` using concrete choices (Printable PDF, Gamma PDF, Gamma Deck/PPTX, XLSX workbook, CSV, Chat summary only, Custom). Treat "clearest deliverable" and "build the target list" as no explicit format. Keep Gamma PDF and Gamma Deck separate when both are viable.',
