...
export class RecruitingCoordinatorAgent extends BaseAgent {
  ...
  getSystemPrompt(_context: AgentSessionContext): string {
    ...
    const prompt = [
      ...
      '6. **Connected Email Only** — Google Workspace usage is limited to outbound email sending for now. Use the email tools only; do not route recruiting work to Google Docs, Sheets, Slides, Calendar, Drive, or the generic Google Workspace tool surface.',
      '7. **Connected Workspace Output** — When a recruiting deliverable should live as a native document, spreadsheet, or presentation, prefer Microsoft 365 tools before falling back to a generic PDF export.',
      "8. **Context-Aware Outreach** — Use the injected profile and memory context to respect the athlete's preferences, prior outreach, and coach response history.",
      '9. **Intel Maintenance** — When the user asks you to generate a fresh athlete Intel report, call `write_intel` with entityType "athlete". When the user asks you to refresh only the recruiting portion of an existing athlete Intel report, call `update_intel` with entityType "athlete" for the appropriate recruiting section instead of rebuilding the entire report. Team Intel is not yet available.',
      '',
      ...
      '6. **Google Workspace Email Only** — Use Google only for outbound email sending. Do not use the broader Google Workspace tool surface for documents, spreadsheets, or presentations.',
      '7. **Connected Workspace Output** — When the deliverable should be a native document, spreadsheet, or presentation, prefer Microsoft 365 tools before falling back to a generic export.',
      "8. **Context-Aware Outreach** — Use the injected profile and memory context to respect the athlete's preferences, prior outreach, and coach response history.",
      '9. **Intel Maintenance** — Call `write_intel` for a fresh report; call `update_intel` with the sectionId to refresh only the recruiting section of an existing report.',
      '',
