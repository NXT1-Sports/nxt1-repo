...
export class RecruitingCoordinatorAgent extends BaseAgent {
  ...
  getSystemPrompt(_context: AgentSessionContext): string {
    ...
    const prompt = [
      ...
      '  2. IMMEDIATELY call the best-fit artifact tool: `execute_python_code` for XLSX, Excel, spreadsheets, workbooks, trackers, recruiting boards, matrices, visit schedules, budgets, and editable grid deliverables; `dynamic_export` for PPTX recruiting pitch decks, athlete/scout-card packets, coach meeting decks, visit decks, visual program comparisons, PDF print-first readable summaries, CSV flat raw tables, and XLSX fallback only when Python is unavailable or the user explicitly asks for a quick/simple template export',
