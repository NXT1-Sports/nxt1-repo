...
function buildCanonicalNotionProperties(
  ...
  if (touchProperty) {
    updates[touchProperty] = { number: repair.finalTouchCount };
  }
...
function describeRepairDrift(repair: RepairPlan): string[] {
  ...
  if ((repair.existingLead?.notionPageUrl ?? null) !== (repair.canonical.pageUrl ?? null)) {
    drift.push(
      `pageUrl:${formatNullable(repair.existingLead?.notionPageUrl)}->${formatNullable(repair.canonical.pageUrl)}`
    );
  }
...
async function main(): Promise<void> {
  ...
  console.log(`[repair-b2b-notion-duplicate-leads] Environment: ${environment.toUpperCase()}`);
  console.log(`[repair-b2b-notion-duplicate-leads] Mode: ${shouldCommit ? 'COMMIT' : 'DRY RUN'}`);
  if (includeSingletons) {
    console.log(
      '[repair-b2b-notion-duplicate-leads] Singleton repair enabled: existing Firestore-backed canonical rows will also be checked for stale Notion fields.'
    );
  }
  ...
  console.log(
    `[repair-b2b-notion-duplicate-leads] Candidate rows with supported stages: ${candidates.length}`
  );
  ...
  console.log(
    `[repair-b2b-notion-duplicate-leads] Duplicate lead groups found: ${candidateGroups.length}`
  );
  if (limit) {
    console.log(
      `[repair-b2b-notion-duplicate-leads] Processing limit: ${limitedGroups.length} groups`
    );
  }
  ...
  for (const group of limitedGroups) {
    ...
    const repair = createRepairPlan(canonical, group, existingLead);
    repairs.push(repair);
  }
  ...
  console.log(`[repair-b2b-notion-duplicate-leads] Impacted groups: ${impacted.length}`);
  ...
  if (!shouldCommit) {
    console.log(
      '[repair-b2b-notion-duplicate-leads] Dry run complete. Re-run with --commit to apply.'
    );
    return;
  }
        ...
        .set(
          {
            ...
            partnerType: repair.canonical.partnerType,
            domain,
            sourceUrl: repair.canonical.sourceUrl,
            primaryContact: repair.canonical.primaryContact ?? null,
            email: repair.email,
  ...
  console.log(`[repair-b2b-notion-duplicate-leads] Firestore docs updated: ${firestoreUpdated}`);
  console.log(
    `[repair-b2b-notion-duplicate-leads] Canonical Notion pages updated: ${notionCanonicalUpdated}`
  );
  console.log(
    `[repair-b2b-notion-duplicate-leads] Duplicate Notion pages archived: ${notionArchived}`
  );
}

main().catch((error) => {
  console.error('[repair-b2b-notion-duplicate-leads] Failed:', error);
  process.exit(1);
});
